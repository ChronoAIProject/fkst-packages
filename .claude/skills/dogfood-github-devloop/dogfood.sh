...
restart_one() {
  ...
  ensure_integration_caught_up "$PKGSRC" "$GH_ORG/fkst-packages"     # keep run branch (integration) >= dev so operator fixes deploy
  [ "$HOST" != "$PKGSRC" ] && ensure_integration_caught_up "$HOST" "$REPO"
...
cmd_sync() {
  ...
  for n in $(expand "${1:-all}"); do
    ...
    if ! ensure_integration_caught_up "$PKGSRC" "$GH_ORG/fkst-packages"; then
      failed=1; continue
    fi
    if [ "$HOST" != "$PKGSRC" ] && ! ensure_integration_caught_up "$HOST" "$REPO"; then
      failed=1; continue
    fi
    st=$(_proc_stale "$n")
    case "$st" in
