...
一个 package = `core.lua`（包内共享库）+ `departments/<dept>/main.lua`（department 入口处理器，暴露 `M.spec` 与 `pipeline(event)`；department **不限于单文件**——职责变大时可在同目录拆出 department-local 子模块，经 `require("departments.<dept>.<mod>")` 引入，`main.lua` 只作入口，如 `autochrono` 的 `propose/mapping.lua`）+ `raisers/<r>.lua`（cron/file_watch 触发器）+ `tests/*_test.lua`。包分两类：flat 平包必须自洽、可单根 conformance、0 外部 package namespace 引用；composed 包是一等包，负责组合/适配兄弟包，可引用 `<pkg>.<queue>`，用 `fkst.toml` 的 `[event_deps]` 声明组合 conformance 需要一起加载的兄弟包。**按 manifest `kind` 分组的权威 package catalog 位于 `README.md` 的 `## Package Catalog`；`CLAUDE.md` 不重复维护该 inventory。**

### `github-devloop` issue 侧 / `github-devloop-pr` PR 侧运行 doctrine

- **共同状态事实**：两侧共用 GitHub 评论里的 trusted-bot `state:v1` marker 推导当前状态与 version（`libraries/devloop/state.lua:22-38,108-139`）；`fkst-dev:<state>` label 由 marker 对应状态投影并在 reconcile 时纠正（`libraries/devloop/state_labels.lua:3-20,141-179`），不是第二状态权威。
- **`github-devloop` = issue 侧**：共识通过后，`core.dependency_gate` 从 GitHub 原生 `issue.blockedBy` 重导依赖，并以 blocker 的 trusted `merged` fact 证明完成（`libraries/devloop/dependency_gate.lua:18-20,333-393`）；只有 `kind="satisfied"` 令 `ok=true`（同文件 `:58-64,614-625`）。`consensus_result` 把未满足 gate 的 issue 推进独立 `dependency_wait` 状态，写 `dependency-wait:v1` / `dependency-cycle:v1` / `dependency-unresolvable:v1` marker 与 `fkst-dev:blocked-on-dependency` label（`packages/github-devloop/departments/consensus_result/main.lua:53-98,319-325`）；后续 replay 对 `waiting` 保持等待，对 `satisfied` 级联到 `ready`，对 `cycle|unresolvable` 确定性转 `blocked`（`packages/github-devloop/core/ready_split.lua:193-270`）。gate 在 `consensus_result`、`observe_issue`、`implement` 三处重导（`packages/github-devloop/departments/consensus_result/main.lua:319-325`、`packages/github-devloop/departments/observe_issue/main.lua:173-186`、`packages/github-devloop/departments/implement/main.lua:590-603`）。`devloop_ready` 由 `implement` 在 runtime worktree 中启动实现并记录 `implementing`；失败结果记录 `impl-failed`，该状态仍有 retry 边（`packages/github-devloop/departments/implement/main.lua:53,87-139,274-290,375-377`、`packages/github-devloop/core/restart/transitions/impl_failed.lua:15-91`）。
- **issue 共识收敛**：`consensus_result` 同步调用 `consensus.reach`，`converge` 时产出 `devloop_consensus_continue`（`packages/github-devloop/departments/consensus_result/main.lua:217-251`）；`loop` 消费该队列，写 `converge-round:v1`，携 `narrowed_question` / findings 重发下一轮 `devloop_consensus_request`（`packages/github-devloop/departments/loop/main.lua:26-35,196-255`）。终止原因仅来自显式 `essence_stall`，或 round≥3 且连续三轮 verdict digest 相同的 true-stall（`libraries/devloop/convergence/rounds.lua:89-129,272-297`）；converge-round 评论写入确认后，`comment_handoff` 才发 `devloop_reconcile`（`packages/github-devloop/departments/comment_handoff/main.lua:145-151`），确定性 `reconcile` 以 `action="drop"` 写 `blocked`，不调用 codex（`packages/github-devloop/departments/reconcile/main.lua:126-205`）。
- **`github-devloop-pr` = PR 侧**：`review_pr` 回源 PR 与 backing issue，并把 issue JSON 与 PR diff 放进 `content_fetch` context bundle 后产出 `devloop_review_request`（`packages/github-devloop-pr/departments/review_pr/main.lua:151-207`、`libraries/devloop/context_bundle.lua:389-396,500-531`）；`review_result` 同步调用 `consensus.reach`，重导 PR open/base/head 后将 effective approve 推进 `merge-ready`，将 effective reject 推进 `fixing` / `review-meta` 或在 fix cap 处交 reconcile（`packages/github-devloop-pr/departments/review_result/main.lua:48-66,106-221,269-318`）。`merge-ready` 评论确认后由 PR 侧 `comment_handoff` 产出 `devloop_merge_ready`（`packages/github-devloop-pr/departments/comment_handoff/main.lua:157-166`）。`fix` 在写前重导 open/same-repo/head，使用非 force ref update，push 后再次校验新 head，再回到 `reviewing`（`packages/github-devloop-pr/departments/fix/main.lua:747-795,536-575`）；`review-meta` 只产 `fixing|blocked`，不产 `merge-ready`（`packages/github-devloop-pr/departments/review_meta/main.lua:30-38,92-133`）。PR review convergence 由 `review_loop` 消费 `devloop_review_continue`，写 `review-converge-round:v1` 并重发 review request；显式 essence-stall / true-stall 发 `devloop_review_reconcile`，PR 侧 `reconcile` 以 `drop` 转 `blocked`（`packages/github-devloop-pr/departments/review_loop/main.lua:26-36,186-272`、`packages/github-devloop-pr/departments/reconcile/main.lua:171-279`）。
- **写姿态与 merge gate**：`FKST_GITHUB_WRITE` 未设为 `1` 时 `write_mode="dry-run"`，设为 `1` 才为 `real`（`libraries/devloop/config.lua:85-87`）；issue 实现发布、PR fix 与 merge 都在各自 effect 前执行该 gate（`packages/github-devloop/departments/implement/main.lua:147-164`、`packages/github-devloop-pr/departments/fix/main.lua:140-152`、`packages/github-devloop-pr/core/merge_executor.lua:430-445,536-545`）。merge 同时要求 trusted `review-result:v1 approve`、head-bound `merge-ready:v1` fact、匹配的 state/version/origin、open same-repo PR、当前 head、merge-queue 次序、高风险证据、mergeability 与 CI gate，并在写前再次重导（`packages/github-devloop-pr/core/merge_executor.lua:62-69,178-221,361-469,470-645`）；执行路径使用普通 `gh pr merge --merge --match-head-commit`，写 `merging` 后确认 merged PR fact，再写 `merged` marker（`libraries/forge/github/entities.lua:199-210`、`libraries/forge/merge/verified_merge.lua:60-118`、`packages/github-devloop-pr/core/merge_executor.lua:646-759`）。own-CI red / 确定不可合并进入 `fixing`；外部 CI 与 pending / unknown gate 事实保持等待并重试（`packages/github-devloop-pr/core/merge_executor.lua:551-619,718-756`）。
...
- **Claim-label doctrine**：`github-devloop` uses one active GitHub claim label as an optimistic lease and UI surface for multi-instance isolation. The default label is `fkst-dev:claimed:<normalized-owner>`; `FKST_GITHUB_CLAIM_LABEL_EXCLUSIVE=1` selects the exclusive bare `fkst-dev:claimed` posture. Acquisition adds the active label, invalidates the local projection, then re-reads the complete claim-label family; any foreign family label wins and stops the appliance. Every guarded external write re-verifies that the exact active label is the only claim-family label present. Losing the claim is stop-on-discovery, while marker trust, version CAS, dedup, review gates, and merge gates remain authoritative. Release is self-label-only: after a fresh label read, the package may remove only its active label, never a foreign claim label; dry-run posture logs the would-release without mutating GitHub.
