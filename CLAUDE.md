...
- **给 oracle 喂 GitHub 链接，而不是把内容粘进 prompt（内容不入 payload 的同一句话）**：oracle 跑在浏览器端 ChatGPT Pro，能自己打开公开可访问的 URL。`ChronoAIProject` 下的 `fkst-packages` / `fkst-substrate` / `fkst-ops` / `fkst-deployments` 目前都是 public，所以 issue / PR / commit / 文件 / diff 一律给链接（`.../pull/N`、`.../pull/N/files`、`.../blob/<sha>/<path>`、`.../commit/<sha>.diff`），让它回源读全貌，而不是贴一份被我截断的快照。**给链接前先核实那个仓当下确实 public**（`gh api repos/<owner>/<repo> --jq .private`）——仓库可见性会变，而私有仓的链接对 oracle 是 404，它会在读不到内容的前提上照样给你一段自信的推理。私有内容只能内联，且内联即受 prompt 体量约束，这本身就是少用内联的理由。接「内容不入 payload」与「回源 derive 真相」：**截断快照违背它，链接不违背。**
