...
- **可靠投递 / durable delivery（substrate dev 已合并）**：投递默认可靠，事件经 redb 持久 delivery（at-least-once-until-ack、lease+fencing、retry+backoff、DLQ）。对包作者：
  ...
  - **【宪法·内容不入 payload】大体量内容（issue body / PR diff / 评论 / 代码 / 文件）绝不整体序列化进可靠投递 payload。** redb 可靠投递 payload 受 ~64KiB 静态上界约束；把内容塞进去会被迫机械截断，导致 codex 看不全，也违背“据 `source_ref` 回源 derive 当前真相”。payload 只承载 `source_ref` 指针 + 小体量控制字段（schema / dedup_key / version / round / 短 digest / opaque fetch context）；需要内容的 codex / department **据 `source_ref` 从源自己 fetch 完整内容**：`gh issue view` 读全 issue + 全部评论、`gh pr diff` 读完整 diff、worktree / 文件系统读代码、网络读资源。新代码不得新增把 body/diff/comments/source bundle 塞进可靠 payload 再截断的设计。
