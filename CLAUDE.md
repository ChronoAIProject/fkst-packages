...
6. **禁止用含糊词代替核实（不要猜·一切讲证据·用户裁定 2026-08-02）**：**「也许 / 可能 / 很可能 / 大概 / 应该是 / 估计 / 我觉得 / 似乎 / probably / likely / presumably / should be / I think / seems」这类含糊词，禁止出现在任何陈述性结论里。** 这些词的实际作用是**把「我没查」伪装成「我查了但有把握程度」**——它让未核实的猜测混进结论、在下游被当成事实使用，且事后无法追责（说错了可以推给「我说的是可能」）。见下方专节。
...
### 不要猜：陈述只有两种合法形态（禁含糊词·实事求是的语言面·HARD GATE）

**任何陈述性结论只允许两种形态，没有第三种：**

1. **已核实事实** — 必须能同时给出**结论 + 证据源**。证据源指得到具体位置：`file:line`、命令与其实际输出、CI/EXIT 码、marker 内容与作者与时间、测量数值与采集方法。
   > 例：`raise_observed_item` 每轮为未变实体重发（`github_poll/main.lua:120-131`），实测 `github_issue_observed` pending=10706。
2. **显式标注的未核实项** — 必须**明说未核实**，并写出**怎样才能核实**。用 `未核实` / `ASSUMED-UNVERIFIED` / `待验证：<具体动作>` 标记。
   > 例：`ASSUMED-UNVERIFIED：终态压缩窗口是否影响再入队 — 未测；核实方法是读 compact_terminal_dead_records 的保留常量。`

**含糊词是第三种形态，正是被禁的那种**——它既没给证据，也没承认没查。写不出证据源就走形态 2，不要用「应该是」把它糊过去。

**这不是禁止表达不确定性——恰恰相反。** 不确定性必须表达，但要用**形态 2 的显式标注**（说清哪一项没核实、怎么核实），而不是用含糊词把不确定性**弥散**进一句听起来像结论的话里。把「不知道」说成「大概是 X」比说「不知道」更有害：前者会被下游当 X 用。**禁词是为了逼出「我不知道，核实方法是 Y」，不是为了逼出虚假的确定性。**

**机械自检（说出 / 写下任何结论前）**：
- 逐句扫含糊词。命中一个 → 停：**要么补上证据源（升为形态 1），要么显式标注未核实并写出核实动作（降为形态 2）。**
- 问「这句话的证据源是什么？」答不出具体位置 = 它不是事实，是猜测。
- **禁止用含糊词包装未做的核实**：与其写「这**可能**是因为 X」，不如写「未核实是否因为 X；核实方法：<命令 / 读哪个文件>」——然后**去执行它**，本仓的核实通常只要 30 秒。

**适用范围**：结论、根因、诊断、issue / PR 正文、commit message、给用户与给下游 worker 的陈述。**不适用**于：明确标注的假设与待验证项（形态 2 本身）、讨论未来不可知（「下一轮可能会 X」——预测不是事实陈述）、引用他人原话。

**为什么是硬规则**：含糊词让「未核实」在文本层面**不可见**，直接击穿机械门 1（「每个事实都要能指到源头」）——因为一句「应该是 X」看不出到底核实没核实，审查者与下游无从分辨，账就此不平。这是「误解/受骗账本」里几乎每条的语言前身：先用含糊词把猜测写进结论，再在下一步被自己当既定事实用。
