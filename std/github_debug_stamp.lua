...
local function read_code_version(git)
  if type(git) ~= "table" or type(git.read_head) ~= "function" then
    return "unknown"
  end
  local ok, result = pcall(git.read_head, { timeout = 30 })
  if not ok then
    return "unknown"
  end
  local head = trim(result)
...
function S.install(M, read_env, git_handle)
  ...
  function M.with_github_debug_stamp(body, context)
    return S.append(body, context, {
      ...
      git = git_handle or production_git(),
    })
  end
end
