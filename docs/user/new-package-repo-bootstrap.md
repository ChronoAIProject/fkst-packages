...
4. 如果是 composed package，在 `packages/<pkg>/composed.deps` 逐行写入需要一起加载做组合 conformance 的兄弟包名；flat package 不要创建 `composed.deps`。
