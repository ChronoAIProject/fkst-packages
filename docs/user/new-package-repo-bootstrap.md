...
1. Create the runtime interface directory `.fkst/` with tracked `.fkst/env.example`, `.fkst/substrate-ref`, and `.fkst/host-profiles/example.env`; ignore `.fkst/packages`, `.fkst/local-packages`, `.fkst/run/`, `.fkst/env`, and concrete `.fkst/host-profiles/*.env` files except the example. Everything else under `.fkst/` is runtime-generated or machine-local.
...
8. Copy `.fkst/env.example` to `.fkst/env` for checkout-local development, or prefer a global profile via `scripts/run.sh host-profile init <name>` for no-repo-pollution host runs. Set `BIN=/path/to/fkst-substrate/target/debug/fkst-framework` when you need an explicit local engine binary; CI checks out engine source from `.fkst/substrate-ref` and builds `fkst-framework`.
