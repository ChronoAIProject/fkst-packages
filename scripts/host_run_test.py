        ...
        try:
            ...
            fake_bin.write_text(
                textwrap.dedent(
                    ...
                    pathlib.Path({json.dumps(str(capture))}).write_text(json.dumps({{"argv": sys.argv, "head": head, "runtime": os.environ.get("FKST_RUNTIME_ROOT"), "durable": os.environ.get("FKST_DURABLE_ROOT"), "project_root": os.environ.get("FKST_PROJECT_ROOT"), "platform_root": os.environ.get("FKST_PLATFORM_ROOT"), "repository_roots": os.environ.get("FKST_CODEX_REPOSITORY_ROOTS")}}, sort_keys=True) + "\\n", encoding="utf-8")
                    """
                ),
                encoding="utf-8",
            )
            ...
            result = h.run_helper(
                textwrap.dedent(
                    ...
                    export FKST_PLATFORM_ROOT=/untrusted/ambient-platform
            ...
            self.assertEqual(Path(payload["platform_root"]).resolve(), source_repo.resolve())
            self.assertEqual(
