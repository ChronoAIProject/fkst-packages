...
class DogfoodDoctorTest(unittest.TestCase):
    ...
    def test_status_and_doctor_use_one_panic_predicate(self) -> None:
        source = DOGFOOD_SCRIPT.read_text(encoding="utf-8")

        self.assertEqual(source.count('panic=$(engine_panic_count "$log")'), 2)
        self.assertNotIn('grep -ac panicked "$log"', source)
