...
def _admission_trace_messages(root: Path, trace_root: Path | None = None) -> list[str]:
    messages: list[str] = []
    for old_relative, new_relative, schema, family, owner in ADMISSION_TRACE_SPECS:
        messages.extend(_trace_pair_messages(
...
def admission_trace_status(root: Path, trace_root: Path | None = None) -> str:
    ...
    emitted = [
        relative
        for _, relative, _, _, _ in ADMISSION_TRACE_SPECS
        if (trace_root / relative).is_file()
    ]
    if not emitted:
