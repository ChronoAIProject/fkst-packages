...
- 输入 schema 是 `consensus.proposal.v1`：必须带 `source_ref` 与 opaque `fetch_context`，可带 `round` / `convergence_question` / bounded `prior_round_digests` / `codex_cwd`；不得带 issue body、PR diff、comments 或 source bundle。`consensus` 不解释 `fetch_context`，只把 `source_ref` + `fetch_context` 原样 relay 进 codex prompt，并指示 codex 回源 fetch 完整当前内容后判断。输出 schema 是 `consensus.consensus_reached.v1` 或 `consensus.consensus_converge.v1`。`consensus_converge` 带 `schema`、`proposal_id`、`round`、`narrowed_question`、bounded `angle_digests`、`dedup_key`、`source_ref`，不带上轮 peer 全文（保持 peer-invisibility）。
...
- 可靠投递事件携带 `source_ref`，下游据此回源 derive 当前事实，不把 proposal payload 当跨 pipeline 真相；大体量内容（issue body / PR diff / 评论 / 代码 / 文件）不进入可靠投递 payload。
...
`packages/github-devloop/` 是组合 `github-proxy` + `consensus` 的 composed 包。它覆盖 issue design consensus、no-consensus 收敛与真停滞 reconcile、ready-to-implement：带 `fkst-dev:enabled` 且最新 GitHub 评论里没有本 proposal 的 `fkst:github-devloop:state:v1` marker 的 issue 会映射成 `consensus.proposal`，proposal 只携带 `source_ref`、短控制字段和 `fetch_context`（指示 codex 用 `gh issue view` 读取完整 issue + 评论），并先写 `thinking` state marker；当 `consensus.consensus_reached` 返回 `approve` / `reject` 时，只有最新 state marker 仍是 `thinking` 才转到 `ready` / `blocked`，发一条带新 state marker 与 `fkst:github-devloop:result:v1` marker 的评论作为外部 durable 记录；`approve` 同时产生包内裸名 `devloop_ready` 事件（schema `github-devloop.ready.v1`）。

当 `consensus.consensus_converge`（带 `narrowed_question` + bounded 角度 digest）到达时，`departments/loop` 回源读取当前 issue marker 与轻量元数据：最新 state marker 仍是 `thinking` 时，从 trusted `fkst:github-devloop:converge-round:v1` marker 求当前 round = max(事件 round, marker round)，写本轮 converge-round marker（记 round + 问题/裁决 digest），并带 `narrowed_question` 以 round+1 重新 raise `consensus.proposal`，让下一轮角度只收窄到分歧点并重新据 `source_ref` fetch 完整当前内容（保持 peer-invisibility，下轮只见收窄问题与 bounded digest，不见上轮 peer 全文）；当 router 判 true-stall（round≥3 且最近连续三轮 question 与 verdicts digest 全相同）时不再重发，而是产生包内裸名 `devloop_reconcile` 事件（schema `github-devloop.reconcile.v1`）。`consensus` 不再有 `consensus_unresolved`、不盲循环、不拆分。
