...
login and head branch name do not determine externality, so a same-repository operator PR is never
bridged and a fork cannot self-exclude by choosing a `devloop/` branch name.
