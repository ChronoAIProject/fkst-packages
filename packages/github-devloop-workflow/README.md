...
- **`on_already_satisfied`** (optional) accepts only `"hold"`. For a later step with an exact
  `result_ready` predecessor, a trusted `already-satisfied` result keeps the origin nonterminal with
  `origin-delivery-unverified`; it does not assert origin delivery. Omitting the field preserves the
  child-fatal result.
