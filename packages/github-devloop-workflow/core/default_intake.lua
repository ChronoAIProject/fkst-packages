...
function M.read_current_for_candidate(package_core, dept, repo, issue_number, candidate, event_ts, expected_decision_dedup_key)
  ...
  if has_pending_reintake and not operator_commands.has_reintake_authority(current.comments, candidate.proposal_id) then
    ...
    devloop_logging.log_cas_decision(dept, candidate.proposal_id, { state = nil, version = nil }, "candidate", "enable|track|decline", "refused(reintake-no-authority)", "operator reintake requires a trusted intake decision or lifecycle state")
