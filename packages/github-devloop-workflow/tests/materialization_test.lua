...
local pred_digest = "d-2222222222"
local spec = {
...
local function key()
  return materialization.materialization_key(origin, blueprint_digest, slot.id, pred_digest)
end
...
local function fact(state, gen_digest)
  return {
    ...
    child_dedup = materialization.child_dedup_key(origin, slot.id, pred_digest),
    child_issue = "108",
    state = state,
  }
end
...
local tests = {
  test_materialization_key_is_deterministic = function()
    t.eq(
      materialization.materialization_key(origin, blueprint_digest, slot.id, pred_digest),
      materialization.materialization_key(origin, blueprint_digest, slot.id, pred_digest)
    )
    t.is_true(materialization.materialization_key(origin, blueprint_digest, slot.id, pred_digest)
      ~= materialization.materialization_key(origin, blueprint_digest, "second", pred_digest))
  end,

  test_child_dedup_key_is_deterministic_from_origin_slot_and_predecessor = function()
    local first = materialization.child_dedup_key(origin, slot.id, pred_digest)
    local second = materialization.child_dedup_key(origin, slot.id, pred_digest)
    t.eq(first, second)
    t.is_true(first:find("github-devloop/issue/owner/repo/42", 1, true) ~= nil)
    t.is_true(first:find("first", 1, true) ~= nil)
    t.is_true(first:find(pred_digest, 1, true) ~= nil)
  end,
  ...
  test_latch_replays_create_when_same_generated_digest_exists = function()
    ...
    t.eq(decision.child_dedup_key, materialization.child_dedup_key(origin, slot.id, pred_digest))
  end,
  ...
  test_write_generated_entry_derives_marker_fields = function()
    local entry = materialization.write_generated_entry(origin, blueprint_digest, slot, pred_digest, spec)
    t.eq(entry.origin, origin)
    t.eq(entry.blueprint_digest, blueprint_digest)
    t.eq(entry.slot, slot.id)
    t.eq(entry.pred_digest, pred_digest)
  ...
  test_marker_roundtrip_fact_key_is_canonical_cas_key = function()
    ...
    local canonical = materialization.materialization_key(origin, blueprint_digest, slot.id, pred_digest)
