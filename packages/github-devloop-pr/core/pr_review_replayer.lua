...
local function replay_review_result(dept, issue, state, facts, tools, link, current_pr)
  local proposal_id = facts.proposal_id
  local fact = facts["review-result"] or markers_facts.review_result_fact(M, comments_for_pr_facts(facts, current_pr), proposal_id, state.version)
  if fact == nil then
    return nil
  end
  ...
  if fact.decision == "approve" then
    local merge_ready = markers_facts.merge_ready_fact(M, comments_for_pr_facts(facts, current_pr), proposal_id, state.version, link.pr_number, current_pr.head_sha)
...
local function replay_review_converge(dept, issue, state, facts, tools, link, current_pr)
  ...
  if review_truth_table_unapproved(latest) then
    local payload = M.build_devloop_review_meta_payload(latest, facts.proposal_id, state.version, link.pr_number, round, latest.source_ref)
...
local function replay_fixing(dept, issue, state, row, facts, tools)
  ...
  if tostring(current_pr.head_sha or "") ~= tostring(feedback.reviewed_head_sha or "") then
    ...
    local fix = {
      ...
      review_dedup_key = feedback.review_dedup_key,
      reviewed_head_sha = feedback.reviewed_head_sha,
      source_ref = M.pr_source_ref(issue.repo, link.pr_number),
    }
    local effects = {
      {
  ...
  local payload = M.build_replayed_fixing_payload({
    proposal_id = proposal_id,
    impl_version = state.version,
  }, link.pr_number, feedback, M.pr_source_ref(issue.repo, link.pr_number))
  ...
  if dept == "observe_pr" then
    local request = fixing_replay_comment_request(issue, link.pr_number, payload, feedback, M.pr_source_ref(issue.repo, link.pr_number))
...
local function review_meta_decision_fact(facts, current_pr)
  return markers_facts.review_meta_decision_fact(M, comments_for_pr_facts(facts, current_pr), facts.proposal_id, facts.state.version)
end
...
local function merge_ready_marker_fact(facts, current_pr)
  return facts.merge_ready or facts["merge-ready"] or markers_facts.merge_ready_fact(M, comments_for_pr_facts(facts, current_pr), facts.proposal_id, facts.state.version, facts.link.pr_number, current_pr.head_sha)
end

local function any_merge_ready_marker_fact(facts, current_pr)
  return facts.merge_ready or facts["merge-ready"] or markers_facts.merge_ready_fact(M, comments_for_pr_facts(facts, current_pr), facts.proposal_id, facts.state.version, facts.link.pr_number, nil)
end
...
local function replay_merge_ready_state(dept, issue, state, row, facts, tools)
  ...
  local approved_ok = markers_facts.review_result_approval_matches_event(M, comments, approved)
...
local function merging_marker_fact(facts, current_pr)
  return facts.merging or markers_facts.merging_fact(M, comments_for_pr_facts(facts, current_pr), facts.proposal_id, facts.link.pr_number, facts.state.version, nil)
end
...
mark_issue_merged_from_linked_pr = function(dept, issue, state, proposal_id, link, pr, tools)
  ...
  local merged_body = comment_strings.comment_string(M, "merged_pr_prefix") .. tostring(link.pr_number)
    ...
    .. "\n" .. markers_builders.merged_marker(M, proposal_id, link.pr_number, state.version, head_sha)
  local source_ref = M.pr_source_ref(issue.repo, link.pr_number)
  local comment_request = M.build_entity_comment_request({
    kind = "issue",
    repo = issue.repo,
...
local function replay_pr_open(dept, issue, state, row, facts, tools)
  ...
  for _, item in ipairs(facts.snapshot.prs or {}) do
    if tostring(item.number or "") == tostring(link.pr_number or "") then
      ...
      if markers_facts.has_any_review_result_marker(M, facts.snapshot.comments, review_proposal_id, proposal_id) then
...
local function replay_reviewing(dept, issue, state, row, facts, tools)
  ...
  if markers_facts.has_any_review_result_marker(M, current_pr.comments, review_proposal_id, proposal_id) then
