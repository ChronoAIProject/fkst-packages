...
function M.should_wait_for_stale_mergeability(core, pr, branches, mergeable_reason, implementation_git)
  if not check_runs.is_not_mergeable_reason(mergeable_reason) then
    return false, "not-stale-mergeability"
  end
  local git = implementation_git or core.git
  local base_head, base_reason = git_mechanics.current_base_head(git, branches.integration)
