...
local function require_consensus_review_approve(comments, merge_ready)
  local ok, reason = markers_facts.review_result_approval_matches_event(core, comments, merge_ready)
  if ok then
    return true
  end
...
local function assert_merge_pr_authority(merge_ready, pr, repo, issue_number, origin, branches)
  ...
  local fact = markers_facts.merge_ready_fact(core, pr.comments, merge_ready.proposal_id, merge_ready.version, merge_ready.pr_number, merge_ready.reviewed_head_sha)
  local approval_ok, approval_reason = markers_facts.merge_ready_approval_matches_event(core, fact, merge_ready)
  if not approval_ok then
    return false, "merge-ready fact changed: " .. tostring(approval_reason), state
  end

  local current_origin = markers_facts.pr_origin_fact(core, pr.comments)
...
local function speculative_fix_fact_for_merge(comments, merge_ready)
  ...
  local fact = markers_facts.merge_gate_fix_fact(core, comments, merge_ready.proposal_id, fix_version)
  if fact == nil or fact.predecessor_set == nil then
    return nil
  end
  if not markers_facts.has_fix_marker(core,
    comments,
...
local function write_merging_marker(repo, merge_ready, comments)
  if markers_facts.merging_fact(core, comments, merge_ready.proposal_id, merge_ready.pr_number, merge_ready.version, merge_ready.reviewed_head_sha) ~= nil then
    return
  end
...
local function process_merge_ready_locked(repo, issue_number, merge_ready, branches, initial_pr, options)
  ...
  if state.state == "merged" and markers_facts.has_merged_marker(core, current_pr.comments, merge_ready.proposal_id, merge_ready.pr_number, merge_ready.version, merge_ready.reviewed_head_sha) then
  ...
  local fact = markers_facts.merge_ready_fact(core, current_pr.comments, merge_ready.proposal_id, merge_ready.version, merge_ready.pr_number, merge_ready.reviewed_head_sha)
  if fact == nil then
  ...
  if not pr_ok then
    if core.is_merged_pr(current_pr)
      ...
      local merging_fact = markers_facts.merging_fact(core, current_pr.comments, merge_ready.proposal_id, merge_ready.pr_number, merge_ready.version, merge_ready.reviewed_head_sha)
  ...
  local merge_ok, merge_reason, merge_rechecked_pr = core.run_verified_pr_merge({
    ...
    validate_rechecked_pr = function(rechecked_pr)
      local recheck_origin = markers_facts.pr_origin_fact(core, rechecked_pr.comments)
