...
function M.make(core, deps)
  ...
  local delivery_target = deps.delivery_target
  ...
  local function process_merge_queue_tick(event)
    local payload = type(event and event.payload) == "table" and event.payload or {}
    local cause = payload.cause
    local cause_kind = type(cause) == "table" and tostring(cause.kind or "") or ""
    local lifecycle_repo = devloop_base.read_env("FKST_GITHUB_REPO")
    if lifecycle_repo == nil or lifecycle_repo == "" then
    ...
    local grants_raw = config.read_env("FKST_DEVLOOP_DELIVERY_GRANTS")
    local requested_repo = select(1, devloop_base.parse_pr_source_ref(payload.source_ref))
    ...
    for _, lane in ipairs(lanes) do
      local repo = lane.implementation_repo
      if requested_repo == nil or requested_repo:lower() == repo:lower() then
        local branches = lane.cross_repo and {
          upstream = lane.implementation_branch,
          integration = lane.implementation_branch,
        } or host_branches
        local lock_key = entity_lib.merge_lane_lock_key(repo)
        if lock_key == nil then
          error("github-devloop: merge-queue-lane-invalid: no transition lock key")
        end
        with_lock(lock_key, function()
          ...
          if target.implementation_repo:lower() ~= repo:lower()
            or tostring(target.implementation_branch) ~= tostring(branches.integration) then
            error("github-devloop: merge-queue-lane-mismatch: durable delivery target differs from implementation lane")
          end
          ...
          local outcome = process_merge_ready_locked(entity.repo, repo, entity.issue_number, merge_ready, branches, nil, {
            enforce_queue = false,
            write_mode = write_mode,
            queue_starvation_cause = cause_kind == "queue-starvation" and cause or nil,
            implementation_git = lane.git,
          })
          if outcome ~= nil and outcome.status == "merged" then
            ...
            if cause_kind ~= "queue-starvation" or selected_is_fifo_head then
              last_merged_pr_number = merge_batch.run_merge_batch_window(core, repo, branches, merge_ready, entries, {
                write_mode = write_mode,
                git = lane.git,
              }, process_merge_ready_locked)
            end
