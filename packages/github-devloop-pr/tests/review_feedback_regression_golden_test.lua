...
return {
  ...
  test_review_feedback_review_meta_output_keeps_origin_shape = function()
    ...
    t.eq(raised[3].queue, "devloop_review_meta")
    t.eq(raised[3].payload.schema, "github-devloop.review-meta.v1")
    t.eq(raised[3].payload.proposal_id, proposal_id)
    t.eq(raised[3].payload.review_proposal_id, review_proposal_id)
    t.eq(raised[3].payload.review_dedup_key, review_dedup_key)
    t.eq(raised[3].payload.version, version)
    t.eq(raised[3].payload.pr_number, 7)
    t.eq(raised[3].payload.n, 0)
    t.eq(raised[3].payload.dedup_key, fix.dedup_key)
    t.eq(raised[3].payload.source_ref.kind, "external")
    t.eq(raised[3].payload.source_ref.ref, "owner/repo#pr/7")
  end,
