...
local function mock_substrate_ref_pin(ref, pin)
  t.mock_command("git show " .. tostring(ref) .. ":.fkst/substrate-ref", {
    stdout = tostring(pin) .. "\n",
    stderr = "",
    exit_code = 0,
  })
...
return {
  ...
  test_merge_missing_substrate_symbol_without_pin_proof_stays_plain_ci_red = function()
    local event = merge_ready()
    local origin_marker = core.pr_origin_marker(event.proposal_id, "42", "devloop-owner-repo-42-01HY", event.version, "dev")
    mock_bot_env()
    mock_write_env("1")
