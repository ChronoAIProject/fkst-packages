...
return {
  test_merge_ready_green_mergeable_records_pr_merged_fact_without_parent_effects = function()
    ...
    local origin_marker = markers_builders.pr_origin_marker(core, event.proposal_id, "42", "devloop-owner-repo-42-01HY", event.version, "dev")
    mock_bot_env()
    mock_write_env("1")
  ...
  test_merge_forged_merging_fact_does_not_finalize = function()
    ...
    table.insert(comments, {
      body = markers_builders.merging_marker(core, event.proposal_id, event.pr_number, event.version, event.reviewed_head_sha),
  ...
  test_merge_gate_feedback_uses_custom_test_command_host_fact = function()
    ...
    local fix_fact = markers_facts.merge_gate_fix_fact(core, { comment_body }, event.proposal_id, core.fix_version_from_review_version(event.version))
  ...
  test_merge_same_second_earlier_review_fact_reenters_reviewing_for_new_head = function()
    ...
    mock_issue_merge({ "fkst-dev:merge-ready" }, {
      ...
      markers_builders.pr_origin_marker(core, event.proposal_id, "42", "devloop-owner-repo-42-01HY", event.version, "dev"),
      {
        body = markers_builders.merge_ready_marker(core, event.proposal_id, event.pr_number, event.version, event.review_proposal_id, event.review_dedup_key, event.reviewed_head_sha),
  ...
  test_merge_stale_idempotent_forged_and_foreign_skip = function()
    ...
    mock_pr_merge({ markers_builders.pr_origin_marker(core, event.proposal_id, "42", "devloop-owner-repo-42-01HY", event.version, "dev") })
    ...
    mock_issue_merge({ "fkst-dev:merged" }, {
      ...
      markers_builders.merged_marker(core, event.proposal_id, event.pr_number, event.version, event.reviewed_head_sha),
    })
    mock_pr_merge({ markers_builders.pr_origin_marker(core, event.proposal_id, "42", "devloop-owner-repo-42-01HY", event.version, "dev") })
    ...
    local forged = markers_builders.merge_ready_marker(core, event.proposal_id, event.pr_number, event.version, event.review_proposal_id, event.review_dedup_key, event.reviewed_head_sha)
    mock_bot_env()
    mock_write_env("1")
    mock_issue_merge({ "fkst-dev:merge-ready" }, {
      core.state_marker(event.proposal_id, "merge-ready", event.version),
