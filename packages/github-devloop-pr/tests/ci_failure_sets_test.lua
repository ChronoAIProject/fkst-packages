...
local function manifest(fields)
  ...
  value.base_commit = fields.base_commit
  value.head_commit = fields.head_commit
  return value
end
...
return {
  ...
  test_failure_identity_rejects_non_string_fields = function()
    local malformed = failure("test_same_assertion")
    malformed.name = 42
    t.eq(ci_failure_sets.failure_identity(malformed), nil)
  end,

  test_equal_failure_sets_have_no_new_failing_identity = function()
  ...
  test_candidate_only_failure_is_new = function()
    local result = compare({ failure("test_k") }, { failure("test_k_prime") })
    t.eq(result.kind, "new-failing-identity")
    t.eq(#result.new_failures, 1)
  end,

  test_mixed_inherited_and_new_failures_is_new = function()
    local result = compare(
      { failure("test_k") },
      { failure("test_k"), failure("test_k_prime") }
    )
    t.eq(result.kind, "new-failing-identity")
    t.eq(#result.new_failures, 1)
  end,

  test_missing_or_incomplete_manifest_is_incomparable = function()
    t.eq(ci_failure_sets.compare_manifests(nil, nil, {}).kind, "incomparable")
    local result = ci_failure_sets.compare_manifests(
      manifest({ tested_commit = BASE_SHA, complete = false }),
      manifest({
        event_name = "pull_request",
        tested_commit = CANDIDATE_SHA,
        base_commit = BASE_SHA,
    ...
    t.eq(result.kind, "incomparable")
  end,

  test_manifest_bound_to_another_base_is_incomparable = function()
    local result = ci_failure_sets.compare_manifests(
      manifest({ tested_commit = BASE_SHA }),
      manifest({
        event_name = "pull_request",
        tested_commit = CANDIDATE_SHA,
  ...
  test_complete_manifest_requires_a_positive_report_count = function()
  ...
  test_complete_manifest_requires_failures_to_be_an_array = function()
    local base = manifest({ tested_commit = BASE_SHA })
    base.failures = { first = failure("test_k") }
    local result = ci_failure_sets.compare_manifests(
      base,
      manifest({
        event_name = "pull_request",
  ...
  test_candidate_manifest_requires_a_full_tested_commit_oid = function()
    local result = ci_failure_sets.compare_manifests(
      manifest({ tested_commit = BASE_SHA }),
      manifest({
        event_name = "pull_request",
