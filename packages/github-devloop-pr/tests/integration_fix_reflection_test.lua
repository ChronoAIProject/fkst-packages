...
return {
  test_review_result_third_fix_round_enters_reflection_checkpoint = function()
    ...
    t.eq(comment_raise.payload.handoff.kind, "github-devloop.review_meta")
    local handed_off = h.run_comment_handoff_from_request(comment_raise.payload, "IC_review_meta_reflection_1", "review-meta-reflection-handoff")
    t.eq(handed_off.exit_code, 0)
    local reflection_raise = find_raise(handed_off.raises, "devloop_review_meta")
    ...
    t.eq(reflection_raise.payload.review_meta_comment_id, "IC_review_meta_reflection_1")
  end,
