...
local function capture(fixture)
  ...
  ra.replace(m_claims, "verify_pr_review_issue_claim", function(_, _, _, _, proposal_id)
    if fixture.claim == false then
      devloop_logging.log_cas_decision("review_meta", proposal_id, { state = nil, version = nil }, "claim", "claim", "skip-claimed-by-other", "backing issue claim label is held by another appliance")
      return false
    end
    return true
  end, restorations)
