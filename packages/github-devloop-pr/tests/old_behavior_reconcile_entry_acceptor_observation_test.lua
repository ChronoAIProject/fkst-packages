...
local FIXTURES = ra.json_array({
  ...
  { disposition = "review-skip-foreign-proposal", status = "rejected", reason = "review-proposal-outside-domain",
    cas = "skip-foreign(proposal_id)", target = "reject", source_line = 168, queue = "devloop_review_reconcile",
    foreign_proposal = true },
  { disposition = "fix-skip-foreign-proposal", status = "rejected", reason = "fix-proposal-outside-domain",
    cas = "skip-foreign(proposal_id)", target = "reject", source_line = 247, queue = "devloop_fix_reconcile",
    foreign_proposal = true },
  { disposition = "review-skip-lock-unavailable", status = "rejected", reason = "review-lock-unavailable",
    cas = "skip-foreign(proposal_id)", target = "reject", source_line = 183, queue = "devloop_review_reconcile",
    no_lock = true },
  { disposition = "fix-skip-lock-unavailable", status = "rejected", reason = "fix-lock-unavailable",
    cas = "skip-foreign(proposal_id)", target = "reject", source_line = 262, queue = "devloop_fix_reconcile",
    no_lock = true },
  { disposition = "timeout-skip-lock-unavailable", status = "rejected", reason = "timeout-lock-unavailable",
    cas = "skip-foreign(proposal_id)", target = "reject", source_line = 370, queue = "devloop_timeout_reconcile",
    no_lock = true },
...
local function event_for(fixture)
  ...
  if fixture.foreign_proposal then payload.proposal_id = "github-devloop/not-an-entity" end
...
local function capture(fixture)
  ...
  function ports.github.pr_cli_view(repo, number, fields, timeout)
    ...
    return { stdout = entity_read_mocks.pr_view_stdout({ repo = REPO, number = PR_NUMBER, comments = comments,
      ...
      base_branch = "dev", state = "OPEN" }), stderr = "", exit_code = 0 }
  end
