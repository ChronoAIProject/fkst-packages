...
return {
  ...
  test_observe_pr_review_meta_fix_marker_advances_to_fixing = function()
    ...
    local comments = {
      ...
      core.state_marker(event.proposal_id, "review-meta", event.version),
      markers_builders.review_meta_marker(core, event.proposal_id, review_dedup_key, "fix", event.version, "missing retry guard"),
    }
    mock_bot_env()
  ...
  test_review_meta_spec_amendment_blocks_without_spawning_intake_issue = function()
    ...
    mock_issue_review_meta({ "fkst-dev:review-meta" }, {
      core.state_marker(event.proposal_id, "review-meta", event.version),
      markers_builders.review_meta_marker(core, event.proposal_id, event.dedup_key, "spec-amendment", core.next_review_meta_action_version(event.version)),
    })
