...
local function review_result_approve_marker(event)
  return markers_builders.review_result_marker(core, event.review_proposal_id, event.proposal_id, "approve", event.review_dedup_key)
end
...
local function append_merged_pr_merging_fact(comments, pr_state)
  ...
  table.insert(merged, markers_builders.merging_marker(core, proposal_id, 7, version, head_sha or "def456"))
  return merged
end
...
local function merge_comments(event, branch, impl_version, include_review_result)
  ...
  local comments = {
    markers_builders.pr_origin_marker(core, event.proposal_id, 42, branch or "devloop-owner-repo-42-01HY", impl_version or version, "dev"),
    core.state_marker(event.proposal_id, "merge-ready", version),
    markers_builders.merge_ready_marker(core, event.proposal_id, event.pr_number, version, event.review_proposal_id, event.review_dedup_key, event.reviewed_head_sha),
  }
...
local function high_risk_review_evidence_marker(event, extra)
  local opts = extra or {}
  return markers_builders.high_risk_review_evidence_marker(core,
...
local function mock_pr_fix(comments, head, head_sha, state, head_repo, cross_repo, times)
  local cached = base.take_pr_phase_comments()
  local with_origin = {
    markers_builders.pr_origin_marker(core, "github-devloop/issue/owner/repo/42", 42, head or "devloop-owner-repo-42-01HY", base.reviewing().version, "dev"),
  }
  local input_comments = comments
  if input_comments == nil or #input_comments == 0 then
    input_comments = cached or {}
...
local function mock_pr_origin_sequence(entries)
  for _, entry in ipairs(entries or {}) do
    ...
    if comments == nil then
      comments = {
        markers_builders.pr_origin_marker(core, "github-devloop/issue/owner/repo/42", "42", entry.head or "devloop-owner-repo-42-01HY", base.reviewing().version, "dev"),
      }
    end
    if cached ~= nil then
      local merged = {}
