...
function M.make(core)
  local function branch_worktree(repo, issue_number, version, branch, git)
    ...
    if existing ~= nil then
      ...
      if dir_result.exit_code == 1 then
        ...
        local remove_result = git.worktree_remove(existing, 60)
    ...
    local fetch_result = git.fetch_branch("origin", branch, 60)
    if fetch_result.exit_code ~= 0 then
      error("github-devloop: git-pr-head-branch-fetch-failed: git PR head branch fetch failed: " .. tostring(fetch_result.stderr))
    end
    local parent = worktree:gsub("/+$", ""):match("^(.*)/[^/]+$") or "."
    local mkdir_result = exec_sync({ cmd = devloop_base.mkdir_p_cmd(parent), timeout = 30 })
    if mkdir_result.exit_code ~= 0 then
      error("github-devloop: worktree-parent-setup-failed: worktree parent setup failed: " .. tostring(mkdir_result.stderr))
    end
    local add_result = git.worktree_add_remote_branch(worktree, "origin", branch, existing ~= nil, 60)
  ...
  local function fetch_expected_pr_merge_product(pr_number, expected_baseline_sha, git)
    if expected_baseline_sha == nil then
      return nil
    end
    local fetch_result = git.fetch_ref("origin", "refs/pull/" .. tostring(pr_number) .. "/merge", 60)
    if fetch_result.exit_code ~= 0 then
  ...
  local function merge_integration_for_fix(worktree, pr_number, integration_branch, expected_baseline_sha, merge_gate_reason, git)
    if core.merge_gate_reason_requires_pr_merge_product(merge_gate_reason) then
      fetch_expected_pr_merge_product(pr_number, expected_baseline_sha, git)
    end
    local base_head = expected_baseline_sha
    if base_head == nil then
      local fetch_result = git.fetch_branch("origin", integration_branch, 60)
      if fetch_result.exit_code ~= 0 then
      ...
      local base_result = git.remote_branch_head("origin", integration_branch, 30)
    ...
    local merge_result = git.merge_no_edit(worktree, base_head, 120)
  ...
  local function merge_sha_for_fix(worktree, sha, context, log_values, git)
    local merge_result = git.merge_no_edit(worktree, sha, 120)
    if merge_result.exit_code == 0 then
      return context
    end
    local unmerged_result = git.unmerged_paths(worktree, 30)
  ...
  local function fetch_verified_pr_head(pr_number, expected_head_sha, git)
    local fetch_result = git.fetch_ref("origin", "refs/pull/" .. tostring(pr_number) .. "/head", 60)
    if fetch_result.exit_code ~= 0 then
  ...
  local function merge_predecessor_entries_for_fix(worktree, integration_branch, predecessors, current_set, git)
    ...
    for _, predecessor in ipairs(predecessors) do
      ...
      local predecessor_head = fetch_verified_pr_head(predecessor.pr_number, predecessor.head_sha, git)
  ...
  local function merge_speculative_predecessors_for_fix(worktree, repo, integration_branch, fix, current_pr, git)
    ...
    return merge_predecessor_entries_for_fix(worktree, integration_branch, predecessors, current_set, git)
  end

  local function assert_no_unmerged_paths(worktree, git)
    local unmerged_result = git.unmerged_paths(worktree, 30)
    if unmerged_result.exit_code ~= 0 then
      error("github-devloop: git-unmerged-path-check-failed: git unmerged path check failed: " .. tostring(unmerged_result.stderr))
    end
  ...
  local function assert_candidate_diff_clean(worktree, reviewed_head_sha, candidate_head_sha, git)
    local check_result = git.diff_check_range(worktree, reviewed_head_sha, candidate_head_sha, 30)
  ...
  local function assert_staged_diff_clean(worktree, git)
    local check_result = git.diff_check(worktree, true, 30)
    if check_result.exit_code == 0 then
      return
    end
