...
local function act_fix(event)
  ...
  local claim_contract = entity.kind == "issue"
    and m_claims.new_label_claim_contract(entity_lib.issue_source_ref(repo, issue_number))
    or nil
  if entity.kind == "issue" and not m_claims.verify_pr_review_issue_claim(
      "fix", repo, issue_number, nil, fix.proposal_id, claim_contract) then
    return
  end
