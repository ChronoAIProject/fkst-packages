...
local function act_liveness_scan(event)
  ...
  local prs, list_deferred = liveness_scan.liveness_scan_list_open_prs(core, repo, timeout, entity_list_cache.entity_list_poll_key(event))
  ...
  local activations, deferred_by_cap, cursor_key = liveness_scan.liveness_scan_activation_slice(repo, "pr", prs, LIVENESS_SCAN_CURSOR_PREFIX)
  local processed = 0
  local attempted = 0

  for _, activation in ipairs(activations) do
    if not sweep_bounds.sweep_has_budget(deadline) then
      liveness_scan.liveness_scan_update_cursor(cursor_key, activations, attempted)
