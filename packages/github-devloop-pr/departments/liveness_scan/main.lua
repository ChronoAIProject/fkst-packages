...
local function act_liveness_scan(event)
  ...
  return with_lock(cursor_lock_key, function()
    local activations, deferred_by_cap, cursor_key, cursor_progress = liveness_scan.liveness_scan_activation_slice(repo, "pr", prs, LIVENESS_SCAN_CURSOR_PREFIX)
    local processed = 0
    local attempted = 0
    ...
    for _, activation in ipairs(activations) do
      ...
      local call_ok, should_reinject, defer_reason = pcall(
        should_reinject_pr,
        repo,
        activation.entity,
        limits,
