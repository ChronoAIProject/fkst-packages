...
return {
  ...
  test_core_doctor_classifies_pr_open_orphan_when_linked_pr_absent = function()
    local current = entity({ "fkst-dev:enabled", "fkst-dev:pr-open" }, {
      ...
      bot_comment(markers_builders.pr_link_marker(core, proposal_id, 7, "devloop/issue/owner/repo/42/v", version, "dev")),
    })
    t.eq(markers_facts.pr_link_fact(core, current.comments, proposal_id).pr_number, 7)
