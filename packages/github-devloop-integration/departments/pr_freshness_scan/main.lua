...
local function candidate_reason(pr, origin, issue, state)
  ...
  if markers_facts.merge_ready_fact(core, pr.comments, origin.proposal_id, state.version, pr.number) ~= nil then
    return "approved"
  end
...
local function process_pr(repo, branches, listed_pr)
  ...
  local origin = markers_facts.pr_origin_fact(core, pr.comments)
