...
function M.gh_issue_assign_cmd(repo, issue_number, login)
  return tostring(repo) .. "#issue/" .. tostring(issue_number) .. "/assign/" .. tostring(login)
end

function M.gh_issue_unassign_cmd(repo, issue_number, login)
  return tostring(repo) .. "#issue/" .. tostring(issue_number) .. "/unassign/" .. tostring(login)
end
...
function M.issue_claim_held_by_self(repo, issue_number, login)
  local view = M.github_handle().rest_issue_view(repo, issue_number, { timeout = 30 })
