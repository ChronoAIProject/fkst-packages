...
return {
  ...
  test_pr_open_request_pushes_creates_comments_and_labels = function()
    ...
    t.eq(result.raises[1].queue, "github_issue_label_request")
    t.eq(result.raises[1].payload.target_kind, "pr")
    t.eq(result.raises[1].payload.issue_number, 7)
    t.eq(result.raises[1].payload.expected_state, "pr-open")
    t.eq(result.raises[1].payload.expected_version, "v1")
    t.eq(result.raises[1].payload.add_labels[1], "fkst-dev:pr-open")
    t.eq(result.raises[2].queue, "github_entity_changed")
    t.eq(result.raises[2].payload.type, "pr")
    t.eq(result.raises[2].payload.repo, "owner/x")
    t.eq(result.raises[2].payload.number, 7)
    t.eq(result.raises[2].payload.source, "github_pr_open")
    t.eq(result.raises[2].payload.source_ref.ref, "owner/x#pr/7")
    t.eq(result.raises[3].queue, "github_pr_opened")
    t.eq(result.raises[3].payload.schema, "github-proxy.pr-opened.v1")
    t.eq(result.raises[3].payload.repo, "owner/x")
    t.eq(result.raises[3].payload.issue_number, 42)
    t.eq(result.raises[3].payload.proposal_id, "github-devloop/issue/owner/x/42")
    t.eq(result.raises[3].payload.impl_version, "v1")
    t.eq(result.raises[3].payload.pr_number, 7)
    t.eq(result.raises[3].payload.branch, "devloop-owner-x-42-01HY")
    t.eq(result.raises[3].payload.head_sha, "abc123")
    t.eq(result.raises[3].payload.base_branch, "dev")
    t.eq(result.raises[3].payload.source_ref.ref, "owner/x#pr/7")
  ...
  test_pr_open_request_allows_current_descendant_head = function()
    ...
    t.eq(count_calls("git push -u origin"), 1)
    t.eq(count_calls("gh pr create"), 1)
    t.eq(find_raise(result.raises, "github_pr_opened").payload.head_sha, "def456")
  end,
  ...
  test_pr_open_request_resolves_created_pr_with_head_list_when_create_stdout_is_unparseable = function()
    ...
    t.eq(#result.raises, 3)
    t.eq(result.raises[1].queue, "github_issue_label_request")
    t.eq(result.raises[1].payload.issue_number, 11)
    t.eq(result.raises[2].queue, "github_entity_changed")
    t.eq(result.raises[2].payload.type, "pr")
    t.eq(result.raises[2].payload.number, 11)
    t.eq(result.raises[3].queue, "github_pr_opened")
    t.eq(result.raises[3].payload.pr_number, 11)
    local issue_written = file.read("/tmp/fkst-github-proxy-pr-open-owner_x-devloop-owner-x-42-01HY-issue-comment.md")
  ...
  test_pr_open_request_reuses_existing_pr_without_duplicate_create = function()
    ...
    t.eq(#result.raises, 3)
    t.eq(result.raises[1].queue, "github_issue_label_request")
    t.eq(result.raises[1].payload.issue_number, 9)
    t.eq(result.raises[2].queue, "github_entity_changed")
    t.eq(result.raises[2].payload.type, "pr")
    t.eq(result.raises[2].payload.number, 9)
    t.eq(result.raises[3].queue, "github_pr_opened")
    t.eq(result.raises[3].payload.pr_number, 9)
    local issue_written = file.read("/tmp/fkst-github-proxy-pr-open-owner_x-devloop-owner-x-42-01HY-issue-comment.md")
