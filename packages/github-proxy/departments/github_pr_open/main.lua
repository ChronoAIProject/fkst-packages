...
local function verify_pr_remote_head(repo, pr_number, expected_head_sha, expected_base_branch)
  local pr_head = core.run_gh(
    "gh pr view head repository/headRefOid/state",
    { cmd = core.gh_pr_view_head_oid_cmd(repo, pr_number), timeout = 30 }
  )
...
local function guard_pr_open_write(repo, payload, bot_login)
  ...
  local view = core.run_gh(
    "gh issue view before PR open",
    { cmd = core.gh_issue_view_pr_open_guard_cmd(repo, payload.issue_number), timeout = 30 }
  )
...
function pipeline(event)
  ...
  with_lock(lock_name(repo, payload.branch), function()
    ...
    local existing = core.run_gh(
      "gh pr list --head",
      { cmd = core.gh_pr_list_head_cmd(repo, payload.branch, payload.base_branch), timeout = 30 }
    )
    ...
    if pr == nil then
      ...
      local created = core.run_gh(
        "gh pr create",
        { cmd = core.gh_pr_create_cmd(repo, payload.branch, payload.base_branch, payload.title, pr_body_path), timeout = 60 }
      )
      pr = core.parse_pr_create(created.stdout)
      if pr == nil then
        local listed = core.run_gh(
    ...
    if not guard.pr_open_visible then
      local issue_view = core.run_gh(
        "gh issue view after PR open",
        { cmd = core.gh_issue_view_comments_cmd(repo, payload.issue_number), timeout = 30 }
      )
    ...
    if not guard.pr_open_visible then
      ...
      core.run_gh(
        ...
        { cmd = core.gh_issue_comment_cmd(repo, payload.issue_number, issue_body_path), timeout = 30 }
      )
    end

    local pr_view = core.run_gh(
      "gh pr view after PR open",
      { cmd = core.gh_pr_view_comments_cmd(repo, pr.number), timeout = 30 }
    )
    if not core.has_trusted_comment_fragment(core.parse_issue_comments(pr_view.stdout), tostring(payload.body), bot_login) then
      ...
      core.run_gh(
        "gh pr comment",
        { cmd = core.gh_pr_comment_cmd(repo, pr.number, pr_body_path), timeout = 30 }
      )
    end
    ...
    if #add_labels > 0 or #remove_labels > 0 then
      with_lock(core.issue_label_lock_key(repo, payload.issue_number), function()
        ...
        core.run_gh("gh issue edit after PR open", {
