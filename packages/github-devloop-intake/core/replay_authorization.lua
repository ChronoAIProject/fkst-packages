...
local function validate_lineage_observation(observed)
  if type(observed) ~= "table" then
    return nil, "observe-lineage-malformed"
  end
  if observed.live_delivery ~= nil and type(observed.live_delivery) ~= "table" then
    return nil, "observe-lineage-malformed"
  end
  if observed.terminal_dead_letter ~= nil and type(observed.terminal_dead_letter) ~= "table" then
    return nil, "observe-lineage-malformed"
  end
  return observed, nil
end

local function read_lineage_observation(observe, source_ref)
  if type(observe) ~= "function" then
    return nil, "observe-lineage-unavailable"
  end
  local reference = source_ref_value(source_ref)
  if type(source_ref) ~= "table"
    or type(source_ref.kind) ~= "string"
    or source_ref.kind == ""
    or type(reference) ~= "string"
    or reference == "" then
  ...
  if not ok then
    return nil, "observe-lineage-unavailable:" .. tostring(observed)
  end
  return validate_lineage_observation(observed)
end
...
local function terminal_identity_equal(left, right)
  return type(left) == "table"
    and type(right) == "table"
    and tostring(left.delivery_id or "") == tostring(right.delivery_id or "")
    and tonumber(left.attempts) == tonumber(right.attempts)
end
...
local function make(deps)
  ...
  function S.revalidate(source_ref, expected_terminal)
    local normalized = base_ids.normalize_source_ref(source_ref)
    local observed, observe_reason = read_lineage_observation(observe, normalized)
    if observed == nil then
      return false, observe_reason
    end
    if observed.live_delivery ~= nil then
      if not matches_lineage(observed.live_delivery, normalized) then
        return false, "observe-lineage-malformed"
      end
      return false, "live-delivery-present"
    end
    local terminal = observed.terminal_dead_letter
    if terminal == nil then
      return false, "terminal-dlq-absent"
    end
    if not is_terminal_tombstone(terminal, normalized) then
      return false, "observe-lineage-malformed"
    end
    if not terminal_identity_equal(terminal, expected_terminal) then
      return false, "terminal-dlq-changed"
    end
    return true, nil
  end
