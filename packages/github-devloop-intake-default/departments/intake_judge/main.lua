...
local function read_current_for_candidate(repo, issue_number, candidate, event_ts, expected_decision_dedup_key)
  ...
  if has_pending_reintake and not operator_commands.has_reintake_authority(current.comments, candidate.proposal_id) then
    ...
    devloop_logging.log_cas_decision("intake_judge", candidate.proposal_id, { state = nil, version = nil }, "candidate", "enable|track|decline", "refused(reintake-no-authority)", "operator reintake requires a trusted intake decision or lifecycle state")
  ...
  if has_pending_reintake and operator_commands.reintake_has_active_devloop_state(current.labels, current.comments, candidate.proposal_id) then
    ...
    devloop_logging.log_cas_decision("intake_judge", candidate.proposal_id, { state = nil, version = nil }, "candidate", "enable|track|decline", "refused(reintake-active-state)", "operator reintake requires a terminal lifecycle state, blocked recovery hold, or no active devloop state")
    devloop_logging.log_raise("intake_judge", candidate.proposal_id, "github-proxy.github_issue_comment_request", refusal)
    return nil
  end
