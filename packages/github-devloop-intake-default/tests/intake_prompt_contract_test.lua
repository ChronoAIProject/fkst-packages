...
return {
  ...
  test_intake_marker_fact_trusts_only_bot_comments = function()
    ...
    local marker = markers_builders.intake_decision_marker(core, proposal_id, "decline", "intake/github-devloop/issue/owner/repo/42/v1", "background")
    t.eq(markers_facts.has_intake_decision_marker(core, { { body = marker, author_login = "ordinary-user" } }, proposal_id), false)
    local fact = markers_facts.intake_decision_fact(core, { { body = marker, author_login = core.trusted_bot_login() } }, proposal_id)
    t.eq(fact.decision, "decline")
    t.eq(fact.service_class, "background")
    t.eq(fact.proposal_id, proposal_id)

    local track_marker = markers_builders.intake_decision_marker(core, proposal_id, "track", "intake/github-devloop/issue/owner/repo/42/v-track", "standard")
    local tracked = markers_facts.intake_decision_fact(core, { { body = track_marker, author_login = core.trusted_bot_login() } }, proposal_id)
    t.eq(tracked.decision, "track")
    t.eq(tracked.service_class, "standard")

    local escalation_marker = markers_builders.intake_decision_marker(core, proposal_id, "escalate-to-class", "intake/github-devloop/issue/owner/repo/42/v2", "standard")
    local escalation = markers_facts.intake_decision_fact(core, { { body = escalation_marker, author_login = core.trusted_bot_login() } }, proposal_id)
    ...
    t.is_nil(markers_facts.intake_decision_fact(core, { { body = missing_class_marker, author_login = core.trusted_bot_login() } }, proposal_id))
    t.is_nil(markers_facts.intake_decision_fact(core, { { body = invalid_class_marker, author_login = core.trusted_bot_login() } }, proposal_id))
  end,
