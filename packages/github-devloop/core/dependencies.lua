...
local function blocker_merged(repo, blocker_number)
  ...
  local link = markers_facts.pr_link_fact(core, current.comments, blocker_proposal_id)
  if link == nil then
  ...
  local origin = markers_facts.pr_origin_fact(core, pr_current.comments)
  if origin == nil
  ...
  local merged = markers_facts.merged_fact(core, pr_current.comments, blocker_proposal_id, link.pr_number, pr_state.version)
...
function M.delegated_blocker_merged(repo, blocker_number, blocker_proposal_id, current, state)
  ...
  local delegation = markers_facts.pr_delegation_fact(core, current.comments, blocker_proposal_id, state.version)
  ...
  local merged = markers_facts.merged_fact(core, pr_current.comments, blocker_proposal_id, delegation.pr_number, delegation.version)
  return merged ~= nil, nil
end
