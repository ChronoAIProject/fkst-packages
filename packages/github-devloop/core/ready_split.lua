...
local function build_ready_split_canonicalized_comment_request(M, repo, issue_number, proposal_id, from_version, to_state, to_version, gate, label_dedup_key, source_ref)
  local state_effects = to_state == "ready" and "result-marker,ready-label,devloop-ready" or "ready-split-canonicalized"
  local body_prefix = "github-devloop ready split canonicalized"
...
function M.raise_ready_split_effects(dept, issue, proposal_id, from_version, to_state, to_version, gate, label_dedup_key, additional_raised)
  ...
  local comment_request = build_ready_split_canonicalized_comment_request(
    M,
    issue.repo,
    issue.number,
    proposal_id,
  ...
  devloop_logging.log_apply(dept, proposal_id, to_state, to_version, {
    add = comment_request.handoff.label_request.add_labels,
    remove = comment_request.handoff.label_request.remove_labels,
  }, emitted)
