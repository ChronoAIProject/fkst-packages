...
return {
  ...
  test_ready_and_implementation_helpers = function()
    ...
    t.eq(core.impl_failure_retry_allowed(local_iteration_fact), true)
    t.eq(core.next_impl_retry_attempt(local_iteration_fact), 2)
    t.eq(core.impl_failure_retry_resets_candidate(local_iteration_fact), true)
    local retried_local_iteration = core.impl_failure_marker(ready.proposal_id, ready.dedup_key, "local-iteration-failed", 2)
    t.eq(core.impl_failure_retry_allowed(core.impl_failure_fact({ retried_local_iteration }, ready.proposal_id, ready.dedup_key)), false)
