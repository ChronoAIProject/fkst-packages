...
local t = h.t
local core = h.core
local LOCK_COMMAND = "FKST_IMPLEMENTATION_WORKTREE_SINGLEFLIGHT_HELPER"
local LOCK_PROTOCOL = "FKST_IMPLEMENTATION_WORKTREE_SINGLEFLIGHT:v1"
...
return {
  test_redrive_drops_before_worktree_preparation_when_verification_is_in_flight = function()
    ...
    t.mock_command(LOCK_COMMAND, {
      stdout = LOCK_PROTOCOL .. ":BUSY:locked\n",
      stderr = "",
      exit_code = 0,
    })
    ...
    t.eq(h.count_calls(LOCK_COMMAND), 1)
  end,
