...
return {
  ...
  test_gh_issue_view_state_command_and_parse = function()
    ...
    local observed_issues = parsers_issue.parse_issue_list_observe('[[{"number":7,"state":"open","title":"Observed","body":"issue body","user":{"login":"fkst-test-bot[bot]"}}]]')
    t.eq(observed_issues[1].number, 7)
    t.eq(observed_issues[1].title, "Observed")
    t.eq(observed_issues[1].body, "issue body")
    t.eq(observed_issues[1].author_login, "fkst-test-bot[bot]")
