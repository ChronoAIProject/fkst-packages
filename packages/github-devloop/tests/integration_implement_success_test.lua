...
return {
  ...
  test_implement_ready_runs_codex_in_worktree_and_marks_implementing = function()
    ...
    t.eq(markers_facts.implementing_fact(core, { state_raise.payload.body }, event.proposal_id, event.dedup_key), nil)
    ...
    local fact = markers_facts.implementing_fact(core, { comment_raise.payload.body }, event.proposal_id, event.dedup_key)
