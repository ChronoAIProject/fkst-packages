...
return {
  ...
  test_implement_ready_runs_codex_in_worktree_and_marks_implementing = function()
    ...
    t.is_true(state_raise.payload.body:find(h.state_comment_request(event.proposal_id, "implementing", event.dedup_key).body, 1, true) ~= nil)
    ...
    t.eq(comment_raise.payload.body:find(h.state_comment_request(event.proposal_id, "implementing", event.dedup_key).body, 1, true), nil)
    ...
    t.eq(outcome_attempt_raise.payload.body:find(h.state_comment_request(event.proposal_id, "implementing", event.dedup_key).body, 1, true), nil)
