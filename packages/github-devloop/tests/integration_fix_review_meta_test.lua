...
return {
  ...
  test_review_loop_unresolved_under_budget_reraises_review_proposal = function()
    ...
    t.is_nil(result.raises[1].payload.diff)
    t.is_nil(result.raises[1].payload.comments)
    t.is_nil(result.raises[1].payload.source_bundle)
    t.is_true(result.raises[1].payload.fetch_context:find("gh pr diff", 1, true) ~= nil)
    t.is_true(result.raises[1].payload.fetch_context:find("Use source_ref external owner/repo#pr/7", 1, true) ~= nil)
