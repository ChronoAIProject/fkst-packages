...
local function find_label_raise(raises, target_kind, state)
  return find_raise(raises, "github-proxy.github_issue_label_request", function(payload)
    return tostring(payload.target_kind or "issue") == tostring(target_kind or "issue")
      and (state == nil or tostring(payload.add_labels and payload.add_labels[1] or "") == "fkst-dev:" .. tostring(state))
  end)
end
...
return {
  ...
  test_merge_queue_poll_drives_current_head_after_non_head_event_held = function()
    ...
    t.eq(find_label_raise(polled.raises, "issue", "merged").payload.add_labels[1], "fkst-dev:merged")
  ...
  test_merge_queue_poll_yields_red_fixing_head_to_next_green = function()
    ...
    t.eq(find_label_raise(first_poll.raises, "issue", "fixing").payload.add_labels[1], "fkst-dev:fixing")
    ...
    t.eq(find_label_raise(second_poll.raises, "issue", "merged").payload.add_labels[1], "fkst-dev:merged")
