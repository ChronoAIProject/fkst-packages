...
local function second_round_comments(event, base_version, command)
  ...
  local comments = {
    h.state_comment_request(event.proposal_id, "impl-failed", replacement_version).body,
...
return {
  ...
  test_malformed_retry_lineage_records_visible_failure_instead_of_crashing = function()
    ...
    local comments = {
      h.state_comment_request(event.proposal_id, "impl-failed", first_replacement_version).body,
  ...
  test_second_reimplement_ready_event_runs_the_retry_instead_of_crashing = function()
    ...
    t.is_true(comment ~= nil)
    t.is_true(comment.payload.body:find(
      h.state_comment_request(event.proposal_id, "implementing", base_version .. "/reimplement/3").body,
      1,
      true
    ) ~= nil)
  end,
