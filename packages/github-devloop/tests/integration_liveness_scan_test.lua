...
return {
  ...
  test_codex_runs_error_fail_safe_defers_without_timeout_decision = function()
    local event = ready()
    local row = core.restart_transition_row("implementing")
    local exec_ref = core.implement_exec_ref(event.proposal_id, event.dedup_key)
    local state = {
    ...
    with_codex_runs(function()
      ...
      for _, log in ipairs(logs) do
        if log.tag == "CODEX_RUNS" and table.concat(log.fields or {}, " "):find("fail-safe-defer", 1, true) ~= nil then
