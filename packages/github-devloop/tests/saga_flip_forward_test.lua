...
return {
  ...
  test_already_raised_open_pr_event_drains_without_git_or_pr_side_effects = function()
    local event = ready()
    local branch = deterministic_branch_for(event)
    local payload = core.build_devloop_open_pr_payload("owner/repo", 42, {
    ...
    local result = run_open_pr(payload, opts("saga-flip-open-pr-drain", { FKST_GITHUB_WRITE = "1" }))

    t.eq(result.exit_code, 0)
    t.eq(#result.raises, 0)
    t.eq(count_calls("show-ref --verify --quiet"), 0)
    t.eq(count_calls("rev-parse --verify"), 0)
    t.eq(count_calls("github-proxy.github_pr_open_request"), 0)
  end,

  test_open_pr_skip_stale_then_implement_liveness_redrive_reaches_awaiting_pr = function()
    local event = ready()
    local branch = deterministic_branch_for(event)
    local payload = core.build_devloop_open_pr_payload("owner/repo", 42, {
      proposal_id = event.proposal_id,
    ...
    local drained = run_open_pr(payload, opts("saga-flip-open-pr-drain-before-redrive", { FKST_GITHUB_WRITE = "1" }))

    t.eq(drained.exit_code, 0)
    t.eq(#drained.raises, 0)
