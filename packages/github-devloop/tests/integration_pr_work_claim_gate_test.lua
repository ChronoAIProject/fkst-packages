...
local function run_decompose_raw(payload, run_opts)
...
return {
  ...
  test_decompose_other_owned_issue_skips_before_marker_or_child_work = function()
    local event = decompose_event()
    mock_bot_env()
    claim(42, { "human" })

    local result = run_decompose_raw(event, opts("decompose-claim-other", {
      FKST_GITHUB_WRITE = "1",
    }))
    t.eq(result.exit_code, 0)
    t.eq(#result.raises, 0)
  ...
  test_decompose_claim_read_failure_fails_closed_before_marker_or_child_work = function()
    local event = decompose_event()
    mock_bot_env()
    failing_claim(42)

    local result = run_decompose_raw(event, opts("decompose-claim-fails", {
      FKST_GITHUB_WRITE = "1",
    }))
    t.eq(result.exit_code, 1)
    t.eq(#result.raises, 0)
