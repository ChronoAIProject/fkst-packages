...
return {
  ...
  test_implement_preserves_a_live_prior_runtime_locked_initializer = function()
    local event = h.ready()
    h.mock_issue_implement({ "fkst-dev:ready", "fkst-dev:thinking" })
    h.mock_locked_initializing_implement_worktree({ initializer_liveness = "live" })

    local result = h.run_implement(event, h.opts("implement-preserve-live-prior-runtime-initializer"))

    t.eq(result.exit_code, 1)
    t.is_true(tostring(result.error):find("initializer is still running", 1, true) ~= nil)
    t.eq(h.count_calls("git worktree remove --force --force"), 0)
    t.eq(h.count_calls("rm -rf --"), 0)
