...
local function mock_wip_stop()
  ...
  t.mock_command(core.gh_issue_view_state_cmd(REPO, WIP_ISSUE_NUMBER), {
    stdout = string.format(
      ...
      render_comment(h.state_comment_request(holder_proposal, "implementing", holder_version).body)
    ),
    stderr = "",
    exit_code = 0,
  })
...
local function mock_case(fixture, event)
  ...
  if fixture.handoff_visible_version ~= nil then
    local visible_marker = h.state_comment_request(PROPOSAL_ID, "ready", fixture.handoff_visible_version).body
