...
-- a typed GitHub read -- the hit / expiry / encoding logic is otherwise invisible (the
...
return {
  test_second_call_within_ttl_returns_cached_without_re_exec = function()
    ...
    local first = require("devloop.github_proxy_entity_view").gh_exec_cached(exec, key, 90)
    t.eq(first.stdout, MULTILINE)
    t.eq(calls, 1)
    local second = require("devloop.github_proxy_entity_view").gh_exec_cached(exec, key, 90)
  ...
  test_expired_entry_refetches = function()
    ...
    local result = require("devloop.github_proxy_entity_view").gh_exec_cached(exec, key, 90)
    t.eq(result.stdout, '{"title":"fresh","author":{"login":"fkst-test-bot"}}', "an expired cache entry must re-fetch, not serve stale")
    t.eq(calls, 1)
  end,

  test_requires_typed_read_function = function()
    local key = "github-devloop/ghread-v2/unittest/owner/repo/9004"
    cache_set(key, "")
    local ok, err = pcall(function()
      require("devloop.github_proxy_entity_view").gh_exec_cached({ argv = { "gh", "issue", "view", "9004" } }, key, 90)
    end)
    t.eq(ok, false)
    t.is_true(tostring(err):find("requires a typed GitHub read function", 1, true) ~= nil)
  end,
