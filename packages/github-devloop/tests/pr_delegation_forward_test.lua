...
return {
  ...
  test_ensure_pr_child_rerun_with_visible_facts_is_idempotent = function()
    ...
    local delegated = markers_builders.pr_delegation_marker(core, issue_proposal, pr_proposal, 7, impl_version, "g1")
    local visible_pr_open = markers_builders.pr_origin_marker(core, issue_proposal, issue_number, branch, impl_version, base_branch)
  ...
  test_child_start_stays_visible_after_child_pr_advances_past_pr_open = function()
    local pr_proposal = "github-devloop/pr/owner/repo/7"
    local delegated = markers_builders.pr_delegation_marker(core, issue_proposal, pr_proposal, 7, impl_version, "g1")
    local advanced_child = markers_builders.pr_origin_marker(core, issue_proposal, issue_number, branch, impl_version, base_branch)
  ...
  test_child_start_dsl_gate_matches_visible_child_start_markers = function()
    local delegated = markers_builders.pr_delegation_marker(core, issue_proposal, pr_proposal(7), 7, impl_version, "g1")
    local matching_origin = markers_builders.pr_origin_marker(core, issue_proposal, issue_number, branch, impl_version, base_branch)
    local mismatched_origin = markers_builders.pr_origin_marker(core, issue_proposal, issue_number, branch .. "-old", impl_version, base_branch)
  ...
  test_existing_delegation_for_different_generation_is_not_current = function()
    local old_delegation = markers_builders.pr_delegation_marker(core, issue_proposal, pr_proposal(7), 7, impl_version, "g1")
  ...
  test_existing_delegation_scan_finds_requested_generation_after_prior_attempt = function()
    local old_delegation = markers_builders.pr_delegation_marker(core, issue_proposal, pr_proposal(7), 7, impl_version, "g1")
    local current_delegation = markers_builders.pr_delegation_marker(core, issue_proposal, pr_proposal(8), 8, impl_version, "g2")
	    local visible_pr_open = markers_builders.pr_origin_marker(core, issue_proposal, issue_number, branch, impl_version, base_branch)
	      .. "\n" .. core.state_marker(issue_proposal, "pr-open", impl_version)
    mock_branch_list(8)
  ...
  test_existing_delegation_same_generation_adopts_pr_but_rewrites_current_version = function()
    ...
    local delegated = markers_builders.pr_delegation_marker(core, issue_proposal, pr_proposal(7), 7, prior_version, "g1")
	    local visible_pr_open = markers_builders.pr_origin_marker(core, issue_proposal, issue_number, branch, impl_version, base_branch)
	      .. "\n" .. core.state_marker(issue_proposal, "pr-open", impl_version)
