...
local function comment(state, marker_version, author_login, created_at, effects)
  local body = (state == "ready" or state == "dependency_wait")
    and h.state_comment_request(proposal_id, state, marker_version, effects).body
    or core.state_marker(proposal_id, state, marker_version, effects)
  return {
