...
return {
  ...
  test_dependency_gate_releases_managed_sibling_with_trusted_pr_merged_marker = function()
    ...
    mock_repo_blocker_issue(sibling_repo, 62, {
      ...
      {
        body = markers_builders.pr_link_marker(core, link.proposal_id, link.pr_number, link.branch, link.impl_version, link.base_branch),
        author_login = "fkst-test-bot",
      },
    })
    mock_repo_blocker_pr(sibling_repo, 63, link, {
      {
        body = markers_builders.pr_origin_marker(core, link.proposal_id, 62, link.branch, link.impl_version, link.base_branch),
        author_login = "fkst-test-bot",
      },
      {
      ...
      {
        body = markers_builders.merged_marker(core, link.proposal_id, 63, "merge-version-7", "def456"),
        author_login = "fkst-test-bot",
      },
    })
    local gate = core.dependency_gate(repo, 42)
