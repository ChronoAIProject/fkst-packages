...
local t = h.t
local loaded, claim_labels_or_error = pcall(require, "devloop.claim_labels")

local function claim_labels()
  if not loaded then
    error("devloop.claim_labels unavailable: " .. tostring(claim_labels_or_error), 0)
  end
  return claim_labels_or_error
end
...
return {
  ...
  test_claim_label_classifier_covers_derived_posture_and_foreign_wins = function()
    ...
    t.eq(labels.classify({}, active), "unassigned")
    t.eq(labels.classify({ active }, active), "self")
    t.eq(labels.classify({ "fkst-dev:claimed:Peer" }, active), "other")
    t.eq(labels.classify({ active, "fkst-dev:claimed:Peer" }, active), "other")
    t.eq(labels.classify({ "fkst-dev:claimedx", "fkst-dev:enabled" }, active), "unassigned")
  end,
