...
local function stale_issue_comments()
  ...
  return {
    trusted_comment(h.state_comment_request(stale_proposal_id, "implementing", stale_version).body),
...
local function healthy_issue_comments()
  return {
    trusted_comment(h.state_comment_request(healthy_proposal_id, "implementing", healthy_version).body),
    trusted_comment(core.implement_attempt_marker(
      healthy_proposal_id,
      healthy_version,
      1,
...
local function created_child_comments()
  return {
    trusted_comment(m_builders.pr_origin_marker(
      ...
      stale_version,
      "dev"
    ) .. "\n" .. h.state_comment_request(stale_proposal_id, "pr-open", stale_version).body),
  }
end
