...
return {
  ...
  test_review_pr_fetch_probe_does_not_embed_diff_payload = function()
    ...
    t.eq(proposal.fetch_context:find(diff_sentinel, 1, true), nil)
    t.eq(proposal.fetch_context:find("ISSUE_BODY_SENTINEL_MUST_NOT_ENTER_PROPOSAL", 1, true), nil)
    t.is_true(proposal.fetch_context:find("Use source_ref external owner/repo#pr/7", 1, true) ~= nil)
    t.is_true(proposal.fetch_context:find("Command: gh pr diff '7' --repo 'owner/repo'", 1, true) ~= nil)
    t.is_true(proposal.fetch_context:find("Do not use proposal payload body, diff, comments, or source_bundle", 1, true) ~= nil)
