...
function H.with_live_codex_runs(run_opts, running, fn)
  ...
  if pid == nil then
    holder:close()
    error("github-devloop test: codex lifetime witness did not report readiness")
  end

  local ok, err = pcall(fn)
  local killed, kill_kind, kill_code = os.execute("kill -TERM " .. tostring(pid))
  if killed ~= true then
    error("github-devloop test: failed to release codex lifetime witness: "
      .. tostring(kill_kind) .. ":" .. tostring(kill_code))
  end
  local closed, close_kind, close_code = holder:close()
  if closed ~= true and close_code ~= 0 then
    error("github-devloop test: codex lifetime witness exited unexpectedly: "
