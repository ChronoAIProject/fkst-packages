...
return {
  test_pr_review_helpers = function()
    ...
    local meta_marker = '<!-- fkst:github-devloop:review-meta:v1 proposal="'
      .. issue_proposal_id
      .. '" dedup="' .. meta_review_dedup
      .. '" action="fix" version="' .. action_version
      .. '" gap="missing retry guard" review_proposal="' .. meta_review_proposal
      .. '" review_dedup="' .. meta_review_dedup
      .. '" head_sha="' .. head_sha .. '" -->'
  ...
  test_review_meta_replay_fact_falls_back_to_state_version = function()
    ...
    local marker = m_builders.review_meta_marker(issue_proposal_id, "consensus:" .. expected_review .. "/review")
  ...
  test_review_meta_replay_fact_falls_back_to_historical_review_reject = function()
    ...
    local marker = m_builders.review_result_marker(expected_review, issue_proposal_id, "reject", expected_dedup, 1, "missing regression guard")
    local fact = core.review_meta_replay_fact({ marker }, issue_proposal_id, issue_version, 7, "def456")
    t.eq(fact.proposal_id, expected_review)
