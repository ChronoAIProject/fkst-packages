...
return {
  ...
  test_consensus_result_result_marker_heals_only_missing_label_and_ready_replay = function()
    ...
    local label = comment.payload.handoff.label_request
    t.eq(label.expected_state, "ready")
    t.eq(label.expected_version, current.dedup_key)
    t.eq(label.marker_guard.expected.state, "ready")
    t.eq(label.marker_guard.expected.version, current.dedup_key)
    t.eq(find_raise(result.raises, "devloop_ready"), nil)
  end,
