...
return {
  ...
  test_implementing_liveness_redrive_uses_current_marker_version = function()
    ...
    t.is_true(comment.payload.body:find(h.state_comment_request(current.proposal_id, "impl-failed", current.dedup_key).body, 1, true) ~= nil)
  end,
  ...
  test_implementing_liveness_redrive_takes_over_orphaned_owner_marker = function()
    ...
    local comments = {
      h.state_comment_request(current.proposal_id, "implementing", current.dedup_key).body,
  ...
  test_liveness_replayer_skips_live_implement_attempt_before_receiver = function()
    ...
    local comments = {
      recent_comment(h.state_comment_request(current.proposal_id, "implementing", current.dedup_key).body),
  ...
  test_observe_skips_live_implement_attempt = function()
    ...
    local comments = {
      recent_comment(h.state_comment_request(event.proposal_id, "implementing", event.dedup_key).body),
      live_implement_attempt_marker(event, run_opts, 1),
    }
    mock_issue_state({ "fkst-dev:enabled", "fkst-dev:implementing" }, "OPEN", comments)
  ...
  test_observe_reraises_reimplement_attempt_preserving_suffix = function()
    ...
    local stuck = {
      h.state_comment_request(event.proposal_id, "implementing", retry_version).body,
