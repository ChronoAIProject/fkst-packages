...
return {
  ...
  test_pr_review_helpers = function()
    ...
    local review_v2 = core.pr_review_proposal_id("owner/repo", 7, version .. "/fix/2", head_sha)
    local digest_one = core.review_reject_framing_digest("same feedback")
    local digest_two = core.review_reject_framing_digest("changed feedback")
    local reject_one = core.review_result_marker(review_v1, "github-devloop/issue/owner/repo/42", "reject", "consensus:" .. review_v1 .. "/review", digest_one, 1)
    local reject_two = core.review_result_marker(review_v2, "github-devloop/issue/owner/repo/42", "reject", "consensus:" .. review_v2 .. "/review", digest_two, 2)
    local digests = core.recent_review_reject_framing_digests({ reject_one, reject_two, marker }, "github-devloop/issue/owner/repo/42", 2)
    t.eq(#digests, 2)
    t.eq(digests[1], digest_two)
    t.eq(digests[2], digest_one)
