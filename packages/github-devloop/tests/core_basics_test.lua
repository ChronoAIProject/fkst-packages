...
return {
  ...
  test_pr_review_helpers = function()
    ...
    local reject_marker = core.review_result_marker(id, "github-devloop/issue/owner/repo/42", "reject", "consensus:v1", "Fix the failing review.")
    t.eq(reject_marker:find("Fix the failing review.", 1, true), nil)
    local reject_fact = core.review_reject_fact({ reject_marker }, "github-devloop/issue/owner/repo/42", version)
    t.is_true(reject_fact.review_reason:find("Fix the failing review.", 1, true) ~= nil)
    local action_version = core.next_review_meta_action_version(version)
    local meta_comment = core.state_marker("github-devloop/issue/owner/repo/42", "fixing", action_version)
      .. "\n" .. core.review_meta_marker("github-devloop/issue/owner/repo/42", "meta-dedup", "fix", action_version, "Run another fix pass.")
