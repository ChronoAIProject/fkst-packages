...
return {
  test_thinking_reconcile_blocks_when_live_version_outranks_convergence_base = function()
    ...
    mock_issue_reconcile({ "fkst-dev:thinking" }, {
      h.state_comment_request(event.proposal_id, "thinking", state_version).body,
    })
  ...
  test_thinking_reconcile_does_not_override_advanced_state = function()
    ...
    mock_issue_reconcile({ "fkst-dev:ready" }, {
      h.state_comment_request(event.proposal_id, "ready", state_version).body,
    })
    ...
    mock_issue_reconcile({ "fkst-dev:implementing" }, {
      h.state_comment_request(event.proposal_id, "implementing", state_version).body,
    })
  ...
  test_implementing_timeout_reconcile_is_neutralized_noop = function()
    ...
    mock_issue_reconcile({ "fkst-dev:implementing" }, {
      {
        body = h.state_comment_request(proposal_id, "implementing", state_version).body,
