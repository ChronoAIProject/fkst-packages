...
local function assert_ready_split_label_request(raises, to_state, to_version, blocked_label_added)
  local label_request = find_raise(raises, "github-proxy.github_issue_label_request", function(payload)
    return payload.expected_state == to_state and payload.expected_version == to_version
  end).payload
  t.is_true(contains_value(label_request.add_labels, "fkst-dev:ready"))
  t.eq(contains_value(label_request.add_labels, devloop_base._blocked_on_dependency_label), blocked_label_added)
  t.eq(contains_value(label_request.remove_labels, devloop_base._blocked_on_dependency_label), not blocked_label_added)
  t.is_true(contains_value(label_request.remove_labels, "fkst-dev:impl-failed"))
  t.eq(label_request.require_marker_guard, true)
  t.eq(label_request.expected_state, to_state)
  t.eq(label_request.expected_version, to_version)
  t.eq(label_request.marker_guard.expected.state, to_state)
  t.eq(label_request.marker_guard.expected.version, to_version)
  if blocked_label_added then
    t.eq(label_request.label_colors[devloop_base._blocked_on_dependency_label], "E99695")
  end
end
...
return {
  ...
  test_implement_backstop_split_generation_uses_inner_ready_version = function()
    ...
    assert_ready_split_label_request(result.raises, "dependency_wait", next_split_version, true)
  end,
  ...
  test_ready_replay_dependency_hold_projects_guarded_state_and_auxiliary_labels = function()
    ...
    local raises = capture_core_raises(function()
      core.replay_ready_state("observe_issue", h.issue(), {
        ...
        current = {
          ...
          comments = {
            trusted_comment("IC_ready_visible", core.state_marker(proposal_id, "ready", version)),
          },
        },
        dependency_gate = {
          ok = false,
    ...
    assert_ready_split_label_request(raises, "dependency_wait", split_version, true)
  end,

  test_legacy_ready_rederive_projects_guarded_ready_state_and_clears_auxiliary_label = function()
    mock_observe_issue(
    ...
    t.eq(result.exit_code, 0)
    assert_ready_split_label_request(result.raises, "ready", core.ready_split_version(version), false)
  end,
  ...
  test_legacy_ready_unresolvable_hold_canonicalizes_to_dependency_wait = function()
    ...
    assert_ready_split_label_request(result.raises, "dependency_wait", split_version, true)
  end,
  ...
  test_dependency_release_ready_handoff_accepts_direct_visible_marker = function()
    ...
    assert_ready_split_label_request(released.raises, "ready", split_version, false)
