...
local function mock_base_probe(outcome, candidate_worktree)
  local probe_worktree = candidate_worktree .. "-base-probe-1-verification-1"
  for _ = 1, 2 do
...
local function run_initial_typed_failure(event, outcome, name, base_outcome)
  ...
  if outcome == "SEMANTIC_FAIL" then
    mock_base_probe(base_outcome or "PASS", worktree)
  end
