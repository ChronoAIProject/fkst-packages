...
local function reconcile_issue_event(event, opts)
  ...
  local cold_observation = v_issue.is_supported_cold_issue_observation(issue)
  if not v_issue.is_supported_issue(issue) and not cold_observation then
  ...
  local function process_issue_event(_, record_authoritative_version)
    ...
    if cold_observation then
      issue.title = current.title
      issue.state = current.state
      issue.labels = current.labels
    end
...
return saga.department(spec, { done = function() return false end, act = function(event)
  queue.dispatch_consumed_queue("observe_issue", spec, event, {
    ...
    ["github-proxy.github_issue_observed"] = function(e)
      if devloop_logging.payload_field(e and e.payload, "cold_replay") == true then
        return reconcile_issue_event(e)
      end
    end,
