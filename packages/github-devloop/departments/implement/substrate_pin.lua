...
local function show_pin(ref, missing_ok)
  local result = support.git().show_file(ref, substrate_ref_path, 30)
...
function M.is_only_pin_delta(base_head, branch)
  ...
  local diff = support.git().diff_name_only(nil, tostring(base_head) .. "..refs/heads/" .. tostring(branch), 30)
