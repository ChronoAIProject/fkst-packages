...
local function raise_impl_failed(repo, issue_number, ready, reason, fault_class, detail, attempt)
  local comment_request = requests_lifecycle.build_impl_failure_comment_request(
    core, repo, issue_number, ready, reason, detail, attempt, fault_class)
...
local function raise_attempt_outcome(repo, issue_number, outcome, publish_authorization)
  ...
  if outcome.kind == "impl-failed" then
    raise_impl_failed(repo, issue_number, outcome.ready, outcome.reason, outcome.fault_class,
      outcome.detail, outcome.attempt)
    return
  end
...
local function process_ready_event(event)
  ...
  with_lock(lock_key, function()
    ...
    if not lineage_ok then
      ...
      raise_impl_failed(repo, issue_number, ready, "invalid-version-lineage", "UNKNOWN",
    ...
    if state.state == "implementing" then
      ...
      if attempts >= MAX_IMPLEMENT_ATTEMPTS then
        ...
        raise_impl_failed(repo, issue_number, marker_ready, "retry-exhausted", "UNKNOWN",
          "No linked PR, remote branch, or local branch progress was visible after " .. tostring(attempts) .. " attempts.", attempts)
        return
      end
