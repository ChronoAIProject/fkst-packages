...
local function raise_result_effects(repo, issue_number, reached, current, state, gate, reason, version, to_state,
  ...
  if granted_effects ~= nil then
    if projected then
      transition_batch = granted_effects[COMMENT_EFFECT_ID]
      label_request = granted_effects[LABEL_EFFECT_ID]
    else
      comment_request = granted_effects[COMMENT_EFFECT_ID]
      label_request = granted_effects[LABEL_EFFECT_ID]
    end
  elseif projected then
    transition_batch = consensus_result_caps.restart_effect_facade.build_consensus_result_batch({
      core = core,
      repo = repo,
    ...
    label_request = requests_labels.build_result_state_label_request(repo, issue_number, reached, to_state)
  else
    comment_request = requests_lifecycle.build_result_comment_request(core, repo, issue_number, reached, to_state)
    label_request = requests_labels.build_result_state_label_request(repo, issue_number, reached, to_state)
  end
  ...
  if projected then
    ...
    elseif not label_matches then
      devloop_logging.log_raise("consensus_result", reached.proposal_id,
        "github-proxy.github_issue_label_request", label_request)
    end
  else
    if not result_marker_visible then
...
local function make_department(ports)
  ...
  local function act_result(event)
    ...
    with_lock(lock_key, function()
      ...
      if first_result ~= nil and first_result.decision ~= reached.decision then
