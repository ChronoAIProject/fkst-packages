...
local function validate_consumes(consumes)
  if type(consumes) ~= "table" or #consumes == 0 then
    error("workflow.saga: nonempty-consumes-required: department requires non-empty consumes")
  end
end

local function validate_spec(spec)
  if type(spec) ~= "table" then
    error("workflow.saga: spec-table-required: department requires spec")
  end
...
local function validate_handlers(handlers)
  if type(handlers) ~= "table" then
    error("workflow.saga: handlers-table-required: department requires handlers")
  end
  if type(handlers.done) ~= "function" then
    error("workflow.saga: done-handler-required: department requires done")
  end
  if type(handlers.act) ~= "function" then
    error("workflow.saga: act-handler-required: department requires act")
  end
end
