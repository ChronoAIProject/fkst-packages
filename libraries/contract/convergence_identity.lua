...
local function require_value(value, field)
  if value == nil or tostring(value) == "" then
    error("contract.convergence_identity: identity-field-missing: missing " .. tostring(field))
  end
...
local function nonnegative_integer(value, field)
  ...
  if number == nil or number < 0 or number ~= math.floor(number) then
    error("contract.convergence_identity: identity-counter-invalid: invalid " .. tostring(field))
  end
  return number
end
...
function I.from_proposal(role, proposal, opts)
  if type(proposal) ~= "table" then
    error("contract.convergence_identity: proposal-invalid: missing proposal")
  end
  opts = opts or {}
