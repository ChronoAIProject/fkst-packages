-- contract.saga_conformance: thin test oracle for saga progress and idempotency.
local C = {}
...
local function validate_case(name, case)
  if type(case) ~= "table" then
    error("contract.saga_conformance: " .. name .. " requires case")
  end
end
...
local function assert_non_empty_table(name, value)
  if type(value) ~= "table" or #value == 0 then
    error("contract.saga_conformance: " .. name .. " requires non-empty table")
  end
end

local function validate_post_condition(step, condition)
  if type(condition) ~= "table" then
    error("contract.saga_conformance: step " .. tostring(step.id) .. " has malformed post_condition")
  end
  if not is_non_empty_string(condition.id) then
    error("contract.saga_conformance: step " .. tostring(step.id) .. " post_condition requires id")
  end
  if not is_non_empty_string(condition.kind) then
    error("contract.saga_conformance: post_condition " .. tostring(condition.id) .. " requires kind")
  end
end

local function validate_step(step)
  if type(step) ~= "table" then
    error("contract.saga_conformance: saga step is malformed")
  end
  if not is_non_empty_string(step.id) then
    error("contract.saga_conformance: saga step requires id")
  end
  if not is_non_empty_string(step.effect) then
    error("contract.saga_conformance: step " .. tostring(step.id) .. " requires effect")
  end
  if not is_non_empty_string(step.request_queue) then
    error("contract.saga_conformance: step " .. tostring(step.id) .. " requires request_queue")
  end
  if type(step.post_conditions) ~= "table" or #step.post_conditions == 0 then
    error("contract.saga_conformance: step " .. tostring(step.id) .. " requires non-empty post_conditions")
  end
...
function C.assert_external_effect_saga(saga_def)
  if type(saga_def) ~= "table" then
    error("contract.saga_conformance: external-effect saga requires definition")
  end
  if not is_non_empty_string(saga_def.id) then
    error("contract.saga_conformance: external-effect saga requires id")
  end
...
local function assert_graphql_add_blocked_by(condition, evidence)
  ...
  require_fragment(
    command,
    "addBlockedBy",
    "contract.saga_conformance: post_condition " .. tostring(condition.id) .. " requires addBlockedBy mutation"
  )
  ...
  require_fragment(
    command,
    blocked_field .. ":",
    "contract.saga_conformance: post_condition " .. tostring(condition.id)
...
local function assert_trusted_comment_marker(condition, evidence)
  ...
  if condition.marker ~= nil then
    require_fragment(
      body,
      condition.marker,
      "contract.saga_conformance: post_condition " .. tostring(condition.id) .. " requires declared marker"
    )
  end
  ...
  if evidence.dedup_key ~= nil then
    require_fragment(
      ...
      "contract.saga_conformance: post_condition " .. tostring(condition.id) .. " requires dedup marker"
    )
  end
  if condition.issue_number_attr ~= nil and evidence.issue_number ~= nil then
    require_fragment(
      ...
      "contract.saga_conformance: post_condition " .. tostring(condition.id) .. " requires issue number marker"
    )
  end
end
...
function C.assert_external_effect_post_condition(condition, evidence)
  if type(condition) ~= "table" then
    error("contract.saga_conformance: external-effect post_condition requires definition")
  end
  ...
  error("contract.saga_conformance: unsupported external-effect post_condition kind " .. tostring(condition.kind))
end

function C.assert_progress(_t, case)
  validate_case("assert_progress", case)
  if type(case.first) ~= "function" then
    error("contract.saga_conformance: assert_progress requires first")
  end
  local before = #fkst.test.command_calls()
  local result = case.first()
  if count_write_calls(before) + count_raises(result) == 0 then
    error("contract.saga_conformance: assert_progress observed no write-class commands")
  end
end
...
function C.assert_idempotent(_t, case)
  ...
  if type(case.first) ~= "function" then
    error("contract.saga_conformance: assert_idempotent requires first")
  end
  if type(case.second) ~= "function" then
    error("contract.saga_conformance: assert_idempotent requires second")
  end
  ...
  if first_effects == 0 then
    error("contract.saga_conformance: assert_idempotent: first delivery made no write-class effect; nothing to prove")
  end
  ...
  if second_effects ~= 0 then
    error("contract.saga_conformance: assert_idempotent observed effects on second delivery")
  end
end
