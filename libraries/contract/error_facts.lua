...
-- Lua's error() prepends "chunkname:line: " unless raised with level 0, so the envelope a handler
-- receives at runtime is position-prefixed while the literal in source is not. Strip one such prefix
-- before matching: the old per-subsystem parsers were unanchored and matched through it, and an
-- anchored parser that did not strip it would classify every runtime error as the fallback.
local function strip_position_prefix(text)
  -- Re-raising with error(err) prepends another "chunkname:line: ", so a message that has crossed
  -- several handlers carries several prefixes. Strip them all; the old per-subsystem parsers were
  -- unanchored and were immune to the nesting depth.
  local current = text
  for _ = 1, 8 do
