...
function M.new(exec, opts)
  ...
  local quota_limiter = quota_admission.new({
    enabled = options.quota_admission,
    now = options.now,
  })
  ...
  function handle._exec(argv, timeout, context, stdout_policy)
    return exec_wrap.run(exec, argv, timeout, context, stdout_policy, handle._trusted_author_policy, quota_limiter)
  end
