...
local function graphql_repo_parts(repo)
  ...
  for _, segment in ipairs({ owner, name }) do
    if segment == nil or segment == "." or segment == ".."
      ...
      error("forge: graphql-repository-invalid: invalid repository")
    end
  end
  return owner, name
end
...
local function sorted_issue_numbers(numbers)
  if type(numbers) ~= "table" then
    error("forge: graphql-issue-numbers-invalid: issue numbers must be a table")
  end
  local seen = {}
  local result = {}
  for _, value in ipairs(numbers) do
    ...
    if number == nil or number < 1 or number > 2147483647 or number ~= math.floor(number) then
      error("forge: graphql-issue-number-invalid: issue number must be a positive integer")
    end
  ...
  table.sort(result)
  if #result == 0 then
    error("forge: graphql-issue-numbers-empty: at least one issue number is required")
  end
  return result
end
