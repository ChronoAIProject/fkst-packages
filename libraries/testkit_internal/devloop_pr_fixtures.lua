...
function M.new(deps)
  deps = deps or {}
  local entity_lib = deps.entity_lib or error("testkit_internal.devloop_pr_fixtures: fixture-dependency-missing: deps.entity_lib is required")
  local base = deps.base or error("testkit_internal.devloop_pr_fixtures: fixture-dependency-missing: deps.base is required")
  local entity_read_mocks = deps.entity_read_mocks
    or error("testkit_internal.devloop_pr_fixtures: fixture-dependency-missing: deps.entity_read_mocks is required")
  local m_builders = deps.m_builders or error("testkit_internal.devloop_pr_fixtures: fixture-dependency-missing: deps.m_builders is required")
  local pr_safety = deps.pr_safety or error("testkit_internal.devloop_pr_fixtures: fixture-dependency-missing: deps.pr_safety is required")
  ...
  local function high_risk_paths_digest()
    if github_risk == nil then
      error("testkit_internal.devloop_pr_fixtures: fixture-dependency-missing: deps.github_risk is required for high-risk helpers")
    end
