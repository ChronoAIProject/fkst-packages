...
function M.new(deps)
  deps = deps or {}
  local h = deps.h
    or error("testkit_internal.restart_edges_conformance_fixtures: fixture-dependency-missing: deps.h is required")
  local entry_inventory = deps.entry_inventory
    or error("testkit_internal.restart_edges_conformance_fixtures: fixture-dependency-missing: deps.entry_inventory is required")
  local restart_cas_catalog = deps.restart_cas_catalog
    or error("testkit_internal.restart_edges_conformance_fixtures: fixture-dependency-missing: deps.restart_cas_catalog is required")
  local restart_edges = deps.restart_edges
    or error("testkit_internal.restart_edges_conformance_fixtures: fixture-dependency-missing: deps.restart_edges is required")
  local expectations = deps.expectations
    or error("testkit_internal.restart_edges_conformance_fixtures: fixture-dependency-missing: deps.expectations is required")

  local core = h.core
  local t = h.t
  local expected_successor_kinds = expectations.successor_kinds
    or error("testkit_internal.restart_edges_conformance_fixtures: fixture-dependency-missing: expectations.successor_kinds is required")
  local expected_real_cas_by_id = expectations.real_cas_by_id
    or error("testkit_internal.restart_edges_conformance_fixtures: fixture-dependency-missing: expectations.real_cas_by_id is required")
  local expected_guard_boundary_cas_by_id = expectations.public_guard_boundary_cas_by_id
  local autonomous_cas_by_id = expectations.autonomous_cas_by_id or {}
  local guard_boundary_cas_by_id = expectations.guard_boundary_cas_by_id or {}
  local timeout_cas_by_id = expectations.timeout_cas_by_id or {}
  local guard_boundary_entitlements_required = expectations.guard_boundary_entitlements_required == true
...
local function timeout_edge_id(owner, state, boundary, output_variant)
  local segments = { owner, state, "timeout" }
  if boundary ~= nil then
    table.insert(segments, boundary)
  end
  table.insert(segments, output_variant)
...
local function assert_edges(actual, expected, empty_rows)
  ...
  for index, expected_edge in ipairs(expected) do
    ...
    if guard_boundary_entitlements_required or expected_edge.transition_effect_entitlements ~= nil then
