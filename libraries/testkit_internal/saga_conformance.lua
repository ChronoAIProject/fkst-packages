...
local function assert_delivery_succeeded(label, ok, result_or_err)
  if not ok then
    error(
      "testkit_internal.saga_conformance: delivery-call-error: "
        .. label
  ...
  if type(result_or_err) == "table"
    ...
    error(
      "testkit_internal.saga_conformance: delivery-exit-unsuccessful: "
        .. label
...
local function validate_case(name, case)
  if type(case) ~= "table" then
    error("testkit_internal.saga_conformance: case-not-table: " .. name .. " requires case")
  end
end
...
local function assert_non_empty_table(name, value)
  if type(value) ~= "table" or #value == 0 then
    error("testkit_internal.saga_conformance: non-empty-table-invalid: " .. name .. " requires non-empty table")
  end
end

local function validate_post_condition(step, condition)
  if type(condition) ~= "table" then
    error("testkit_internal.saga_conformance: post-condition-not-table: step " .. tostring(step.id) .. " has malformed post_condition")
  end
  if not is_non_empty_string(condition.id) then
    error("testkit_internal.saga_conformance: post-condition-id-invalid: step " .. tostring(step.id) .. " post_condition requires id")
  end
  if not is_non_empty_string(condition.kind) then
    error("testkit_internal.saga_conformance: post-condition-kind-invalid: post_condition " .. tostring(condition.id) .. " requires kind")
  end
end

local function validate_step(step)
  if type(step) ~= "table" then
    error("testkit_internal.saga_conformance: saga-step-not-table: saga step is malformed")
  end
  if not is_non_empty_string(step.id) then
    error("testkit_internal.saga_conformance: saga-step-id-invalid: saga step requires id")
  end
  if not is_non_empty_string(step.effect) then
    error("testkit_internal.saga_conformance: saga-step-effect-invalid: step " .. tostring(step.id) .. " requires effect")
  end
  if not is_non_empty_string(step.request_queue) then
    error("testkit_internal.saga_conformance: saga-step-request-queue-invalid: step " .. tostring(step.id) .. " requires request_queue")
  end
  if type(step.post_conditions) ~= "table" or #step.post_conditions == 0 then
    error("testkit_internal.saga_conformance: saga-step-post-conditions-invalid: step " .. tostring(step.id) .. " requires non-empty post_conditions")
  end
...
function C.assert_external_effect_saga(saga_def)
  if type(saga_def) ~= "table" then
    error("testkit_internal.saga_conformance: saga-definition-not-table: external-effect saga requires definition")
  end
  if not is_non_empty_string(saga_def.id) then
    error("testkit_internal.saga_conformance: saga-id-invalid: external-effect saga requires id")
  end
...
local function assert_graphql_add_blocked_by(condition, evidence)
  ...
  require_fragment(
    command,
    "addBlockedBy",
    "testkit_internal.saga_conformance: graphql-mutation-missing: post_condition " .. tostring(condition.id) .. " requires addBlockedBy mutation"
  )
  ...
  require_fragment(
    command,
    blocked_field .. ":",
    "testkit_internal.saga_conformance: graphql-field-missing: post_condition " .. tostring(condition.id)
  ...
  for _, forbidden in ipairs(condition.forbidden_fields or {}) do
    if command:find(tostring(forbidden), 1, true) ~= nil then
      error(
        "testkit_internal.saga_conformance: forbidden-graphql-field-present: post_condition " .. tostring(condition.id)
...
local function assert_trusted_comment_marker(condition, evidence)
  ...
  if type(body) ~= "string" or body == "" then
    error(
      "testkit_internal.saga_conformance: marker-body-evidence-invalid: post_condition " .. tostring(condition.id)
  ...
  if condition.marker ~= nil then
    require_fragment(
      body,
      condition.marker,
      "testkit_internal.saga_conformance: declared-marker-missing: post_condition " .. tostring(condition.id) .. " requires declared marker"
    )
  end
  for _, fragment in ipairs(condition.required_body_fragments or {}) do
    require_fragment(
      body,
      fragment,
      "testkit_internal.saga_conformance: marker-fragment-missing: post_condition " .. tostring(condition.id)
  ...
  if evidence.dedup_key ~= nil then
    require_fragment(
      ...
      "testkit_internal.saga_conformance: dedup-marker-missing: post_condition " .. tostring(condition.id) .. " requires dedup marker"
    )
  end
  if condition.issue_number_attr ~= nil and evidence.issue_number ~= nil then
    require_fragment(
      ...
      "testkit_internal.saga_conformance: issue-number-marker-missing: post_condition " .. tostring(condition.id) .. " requires issue number marker"
    )
  end
end
...
function C.assert_external_effect_post_condition(condition, evidence)
  if type(condition) ~= "table" then
    error("testkit_internal.saga_conformance: external-effect-post-condition-not-table: external-effect post_condition requires definition")
  end
  ...
  error("testkit_internal.saga_conformance: external-effect-post-condition-kind-unsupported: unsupported external-effect post_condition kind " .. tostring(condition.kind))
end

function C.assert_progress(_t, case)
  validate_case("assert_progress", case)
  if type(case.first) ~= "function" then
    error("testkit_internal.saga_conformance: first-delivery-handler-invalid: assert_progress requires first")
  end
  local classifier = write_classifier(case)
  if classifier == nil then
    error("testkit_internal.saga_conformance: write-classifier-missing: assert_progress requires is_write_class")
  end
  ...
  if count_write_calls(before, classifier) + count_raises(result) == 0 then
    error("testkit_internal.saga_conformance: progress-effect-missing: assert_progress observed no write-class commands")
  end
end
...
function C.assert_idempotent(_t, case)
  ...
  if type(case.first) ~= "function" then
    error("testkit_internal.saga_conformance: first-delivery-handler-invalid: assert_idempotent requires first")
  end
  if type(case.second) ~= "function" then
    error("testkit_internal.saga_conformance: second-delivery-handler-invalid: assert_idempotent requires second")
  end
  local classifier = write_classifier(case)
  if classifier == nil then
    error("testkit_internal.saga_conformance: write-classifier-missing: assert_idempotent requires is_write_class")
  end
  ...
  if first_effects == 0 then
    error("testkit_internal.saga_conformance: first-delivery-effect-missing: assert_idempotent: first delivery made no write-class effect; nothing to prove")
  end
  ...
  if second_effects ~= 0 then
    error("testkit_internal.saga_conformance: second-delivery-not-idempotent: assert_idempotent observed effects on second delivery")
  end
end
