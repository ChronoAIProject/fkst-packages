  ...
  for _, item in ipairs(decoded) do
    if type(item) == "table" and tonumber(item.number) ~= nil then
      table.insert(items, {
        ...
        labels = shared.label_names(nil, item.labels),
      })
    end
  end
  return items
...
local function recurrence_label_digest(labels)
  local selected = {}
  ...
  return "labels=" .. first_chars(table.concat(selected, ","), 120)
end

local function state_label(labels)
  for _, label in ipairs(labels or {}) do
    local text = tostring(label)
    if devloop_state.is_state_label(text) then
      return text
    end
  end
  return "open"
...
local function render_closed_issue_line(item)
  return "#" .. tostring(item.number)
    .. " [closed] "
    .. first_chars(item.title, 80)
    .. " (" .. recurrence_label_digest(item.labels) .. ")"
end

local function render_board_digest(issues, prs, closed_issues)
  local lines = {
    devloop_base._untrusted_issue_data_begin,
    "Open items snapshot:",
  }
  ...
  for _, item in ipairs(closed_issues or {}) do
    ...
    table.insert(lines, render_closed_issue_line(item))
  end
  if type(closed_issues) ~= "table" or #closed_issues == 0 then
    table.insert(lines, "(none fetched)")
  end
  table.insert(lines, devloop_base._untrusted_issue_data_end)
  return table.concat(lines, "\n")
end

local function fetch_board_feed()
  local cmd = board_feed_cmd()
  if cmd == nil then
    return nil
  end
  ...
  return devloop_base._untrusted_issue_data_begin .. "\n"
    .. "Board feed-through from FKST_DEVLOOP_BOARD_CMD:\n"
    .. stdout:gsub("%s*$", "")
    .. "\n" .. devloop_base._untrusted_issue_data_end
end

function C.board_digest_block(repo, tick)
  if tick == nil or tostring(tick) == "" then
    return ""
  end
  local key = "github-devloop/board-digest/" .. base_ids.safe_repo(repo) .. "/" .. devloop_base.safe_updated_at(tick)
  local cached = cache_get(key)
  if cached ~= nil and cached ~= "" then
    return cached
  end

  local feed = fetch_board_feed()
  if feed ~= nil then
    cache_set(key, feed)
    return feed
  end

  local github_port = shared.github(nil)
...
function C.append_board_digest_to_proposal(proposal, repo, tick)
  local block = C.board_digest_block(repo, tick)
  if block == "" then
    return proposal
  end
  ...
  local remaining = devloop_base._max_body_len - #body - #prefix
  if remaining <= 0 then
  ...
  if #proposal.body > devloop_base._max_body_len then
