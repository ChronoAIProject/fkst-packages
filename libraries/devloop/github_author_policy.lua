...
local function configured_bot_login_or_nil(exec)
  local bot_login = nil
  ...
  return bot_login ~= "" and devloop_base.strip_bot_login_suffix(bot_login) or nil
end

local function configured_bot_login(exec)
  local bot_login = configured_bot_login_or_nil(exec)
...
function M.from_env(exec, github_handle, bot_login)
  local resolved_handle = resolve_github_handle(github_handle)
  bot_login = bot_login or configured_bot_login(exec)
...
function M.github_options(exec)
  ...
  local function resolve_credential(required)
    if not credential_resolved then
      credential_scope = configured_bot_login_or_nil(exec)
      credential_resolved = true
    end
    if required and credential_scope == nil then
      return configured_bot_login(exec)
    end
  ...
  return {
    trusted_author_policy = function(github_handle)
      if policy == nil then
        policy = M.from_env(exec, github_handle, resolve_credential(true))
      end
      return policy
    end,
    credential_scope = function()
      return resolve_credential(false)
    end,
  }
end
