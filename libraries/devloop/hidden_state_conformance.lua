...
local function child_pr(core, state, child_state)
  local body = markers_builders.pr_origin_marker(core, ISSUE_PROPOSAL, ISSUE_NUMBER, BRANCH, state.version, BASE_BRANCH)
  if child_state ~= nil then
    body = body .. "\n" .. core.state_marker(PR_PROPOSAL, child_state, state.version)
  end
  if child_state == "merged" then
    body = body .. "\n" .. markers_builders.merged_marker(core, PR_PROPOSAL, PR_NUMBER, state.version, HEAD_SHA)
  end
  return {
    repo = REPO,
    number = PR_NUMBER,
...
local function add_common_pr_facts(core, entity, state, facts, include_pr_link_marker)
  ...
  if include_pr_link_marker == true then
    facts["pr-link"] = link
    table.insert(entity.comments, comment(core, markers_builders.pr_link_marker(core, ISSUE_PROPOSAL, PR_NUMBER, BRANCH, state.version, BASE_BRANCH), "2026-06-03T01:03:03Z"))
  else
...
local function install_marker(core, entity, state, family, value, is_synthetic)
  ...
  elseif family == "implementing" then
    table.insert(entity.comments, comment(core, markers_builders.implementing_marker(core, ISSUE_PROPOSAL, state.version, BRANCH, HEAD_SHA, BASE_BRANCH, BASE_SHA), "2026-06-03T01:03:07Z"))
  elseif family == "impl-failure" then
  ...
  elseif family == "fix-feedback" then
    table.insert(entity.comments, comment(core, markers_builders.merge_gate_marker(core, ISSUE_PROPOSAL, PR_NUMBER, state.version, value.review_proposal_id, value.review_dedup_key, value.reviewed_head_sha, BASE_SHA, value.reason or "behavioral-fixture"), "2026-06-03T01:03:09Z"))
  elseif family == "review-result" then
    table.insert(entity.comments, comment(core, markers_builders.review_result_marker(core, value.review_proposal_id, ISSUE_PROPOSAL, value.decision or "reject", value.review_dedup_key, core.version_fix_round(state.version), value.blocking_gap or "behavioral-fixture"), "2026-06-03T01:03:09Z"))
    if value.decision == "approve" then
      table.insert(entity.comments, comment(core, markers_builders.merge_ready_marker(core, ISSUE_PROPOSAL, PR_NUMBER, state.version, value.review_proposal_id, value.review_dedup_key, HEAD_SHA), "2026-06-03T01:03:09Z"))
    else
      table.insert(entity.comments, comment(core, markers_builders.merge_gate_marker(core, ISSUE_PROPOSAL, PR_NUMBER, state.version, value.review_proposal_id, value.review_dedup_key, HEAD_SHA, BASE_SHA, value.blocking_gap or "behavioral-fixture"), "2026-06-03T01:03:09Z"))
    end
  elseif family == "review-meta" then
    table.insert(entity.comments, comment(core, markers_builders.review_meta_marker(core, ISSUE_PROPOSAL, value.review_dedup_key, value.action, state.version, value.blocking_gap or "behavioral-fixture"), "2026-06-03T01:03:09Z"))
  ...
  elseif family == "merge-ready" then
    if value.approve ~= false then
      table.insert(entity.comments, comment(core, markers_builders.review_result_marker(core, value.review_proposal_id, ISSUE_PROPOSAL, "approve", value.review_dedup_key), "2026-06-03T01:03:08Z"))
    end
    table.insert(entity.comments, comment(core, markers_builders.merge_ready_marker(core, ISSUE_PROPOSAL, PR_NUMBER, state.version, value.review_proposal_id, value.review_dedup_key, HEAD_SHA), "2026-06-03T01:03:09Z"))
  elseif family == "merging" then
    table.insert(entity.comments, comment(core, markers_builders.review_result_marker(core, value.review_proposal_id or review_proposal(core, state), ISSUE_PROPOSAL, "approve", value.review_dedup_key or review_dedup(core, state)), "2026-06-03T01:03:07Z"))
    table.insert(entity.comments, comment(core, markers_builders.merge_ready_marker(core, ISSUE_PROPOSAL, PR_NUMBER, state.version, value.review_proposal_id or review_proposal(core, state), value.review_dedup_key or review_dedup(core, state), HEAD_SHA), "2026-06-03T01:03:08Z"))
    table.insert(entity.comments, comment(core, markers_builders.merging_marker(core, ISSUE_PROPOSAL, PR_NUMBER, state.version, HEAD_SHA), "2026-06-03T01:03:09Z"))
...
local function add_context_facts(core, row, entity, state, facts, source_ref, include_fact, declared)
  if row.from_state == "awaiting-pr" then
    ...
    table.insert(entity.comments, comment(core, markers_builders.pr_delegation_marker(core, ISSUE_PROPOSAL, PR_PROPOSAL, PR_NUMBER, state.version, "g1"), "2026-06-03T01:03:03Z"))
    facts.pr_delegation = {
      proposal_id = ISSUE_PROPOSAL,
