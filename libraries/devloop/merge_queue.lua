...
local function merge_queue_entry_from_pr(M, repo, pr_number, pr, expected_base)
  ...
  local fact = markers_facts.merge_ready_fact(M, pr.comments, state.proposal_id or "", merge_ready_version_for_lane_state(M, state), pr_number, current_head_sha)
  if fact == nil then
      ...
      for marker in parsers_misc._comment_body(M, comment):gmatch("<!%-%- fkst:github%-devloop:merge%-ready:v1.-%-%->") do
        ...
        if marker_issue ~= nil then
          ...
          if merge_queue_lane_states[candidate_state.state]
            ...
            fact = markers_facts.merge_ready_fact(M, pr.comments, marker_issue, merge_ready_version, pr_number, current_head_sha)
...
function M.wip_admission_classification(repo, proposal_id, issue_comments, state, integration_branch)
  ...
  local link = markers_facts.pr_link_fact(M, issue_comments, proposal_id)
