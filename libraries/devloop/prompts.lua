...
function P.build_implement_prompt(proposal_id, current, framing, content_manifest, result_context)
  local prompt = load_prompt("implement")
  local local_test_command = config.local_iteration_test_command()
  local context = result_context or {}
  ...
  return rendered .. "\n\n" .. devloop_base.render_template(prompt.result_contract, {
    proposal_id = devloop_base.neutralize_untrusted_prompt_text(proposal_id),
