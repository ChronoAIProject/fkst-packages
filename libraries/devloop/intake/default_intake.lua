...
local function raise_enable_successor(package_core, dept, repo, issue_number, candidate, current, event_ts, decision_dedup_key, options)
  ...
  if not v_execution_request.is_supported_execution_request(package_core, execution_request) then
...
local function read_current_for_candidate(package_core, dept, repo, issue_number, candidate, event_ts, expected_decision_dedup_key)
  ...
  local reintake_command = operator_commands.operator_command_fact(package_core, current.comments, "reintake")
  local has_pending_reintake = reintake_command ~= nil and not operator_commands.has_operator_command_response(package_core, current.comments, reintake_command)
  if has_pending_reintake and not m_facts.has_intake_decision_marker(package_core, current.comments, candidate.proposal_id) then
  ...
  local intake_fact = m_facts.intake_decision_fact(package_core, current.comments, candidate.proposal_id)
...
function M.act(package_core, event, opts)
  ...
  if not v_intake_candidate.is_supported_intake_candidate(package_core, candidate) then
  ...
  if type(opts.before_codex) == "function" then
    local handled = opts.before_codex(ctx)
    if handled then
      return
    end
  end
  ...
  with_lock(lock_key, function()
    ...
    local command_comment_request = has_pending_reintake
      and operator_commands.build_operator_issue_reintake_comment_request(package_core, repo, issue_number, reintake_command, candidate, candidate.source_ref)
      or nil
    local raised = {
    ...
    if enables_pipeline(parsed.action) then
      table.insert(apply_add, 1, package_core._enabled_label)
    elseif tracks_umbrella(parsed.action) then
      table.insert(apply_add, 1, package_core._tracking_label)
    end
