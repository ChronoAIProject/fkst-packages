...
local function child_pr_delegation_fact(M, facts)
  return facts.pr_delegation
    ...
    or markers_facts.pr_delegation_fact(M, facts.snapshot.comments, facts.proposal_id, facts.state and facts.state.version)
end
...
local function require_marker_fact(M, facts, family)
  ...
  if family == "pr-link" then
    return markers_facts.pr_link_fact(M, facts.snapshot.comments, facts.proposal_id) or (facts._synthetic_pr_link ~= true and facts.link or nil)
  end
  ...
  if family == "review-result" then
    return markers_facts.review_reject_fact(M, facts.snapshot.comments, facts.proposal_id, facts.state.version)
  end
  ...
  if family == "review-meta" then
    ...
    return markers_facts.review_meta_fix_fact(M, facts.snapshot.comments, facts.proposal_id, facts.state.version)
  end
  ...
  if family == "merge-gate" then
    return markers_facts.merge_gate_fix_fact(M, facts.snapshot.comments, facts.proposal_id, facts.state.version)
  end
  ...
  if family == "implementing" then
    return markers_facts.implementing_fact(M, facts.snapshot.comments, facts.proposal_id, facts.state.version)
  end
  ...
  if family == "merge-ready" then
    ...
    return markers_facts.merge_ready_fact(M, facts.snapshot.comments, facts.proposal_id, facts.state.version, facts.link.pr_number, current_pr.head_sha)
  end
  if family == "merging" then
    ...
    return markers_facts.merging_fact(M, facts.snapshot.comments, facts.proposal_id, facts.link.pr_number, facts.state.version, current_pr.head_sha)
  end
...
local function gather_fetch_before_compare_fact(M, facts, entity, family)
  if family == "pr-head" then
    if facts.link ~= nil and facts.current_pr ~= nil then
      ...
      facts.link = markers_facts.pr_link_fact(M, facts.snapshot.comments, facts.proposal_id)
    end
    return true
  end
...
local function gather_required_facts(M, row, entity, state, provided)
  ...
  gathered.link = gathered.link or markers_facts.pr_link_fact(M, gathered.snapshot.comments, gathered.proposal_id)
...
local function raise_reviewing_for_current_head(M, dept, issue, state, proposal_id, link, current_pr, outcome, reason)
  ...
  if not dept_can_direct_reviewing(dept) then
    local merge_ready = markers_facts.merge_ready_fact(M, current_pr.comments, proposal_id, state.version, link.pr_number)
    local comment_request = M.build_merge_head_reviewing_comment_request(
      issue.repo,
      issue.number,
      {
...
local function maybe_replay_review_carry_over(M, dept, issue, state, row, facts, link, current_pr)
  ...
  if markers_facts.has_any_review_result_marker(M, current_pr.comments, carry.new_review_proposal_id, proposal_id) then
    return false
  end
  local source_ref = M.pr_source_ref(issue.repo, link.pr_number)
...
local function replay_merge_ready_like(M, tools, dept, issue, state, row, facts)
  ...
  local fact = markers_facts.merge_ready_fact(M, facts.snapshot.comments, proposal_id, state.version, link.pr_number, current_pr.head_sha)
  if fact == nil then
